...
const int VZZX_CENTER_QUORUM = 2;
const int VZZX_FINDER_PATTERN_MIN_SKIP = 3;
const int VZZX_FINDER_PATTERN_MAX_MODULES = 57;

@interface VZZXQRCodeFinderPatternFinder ()
...
@implementation VZZXQRCodeFinderPatternFinder

- (id)initWithImage:(VZZXBitMatrix *)image {
  return [self initWithImage:image resultPointCallback:nil];
}

- (id)initWithImage:(VZZXBitMatrix *)image resultPointCallback:(id<VZZXResultPointCallback>)resultPointCallback {
  if (self = [super init]) {
    _image = image;
...
- (VZZXQRCodeFinderPatternInfo *)find:(VZZXDecodeHints *)hints error:(NSError **)error {
  ...
  int iSkip = (3 * maxI) / (4 * VZZX_FINDER_PATTERN_MAX_MODULES);
  if (iSkip < VZZX_FINDER_PATTERN_MIN_SKIP || tryHarder) {
    iSkip = VZZX_FINDER_PATTERN_MIN_SKIP;
  }
  ...
  for (int i = iSkip - 1; i < maxI && !done; i += iSkip) {
    ...
    for (int j = 0; j < maxJ; j++) {
      if ([self.image getX:j y:i]) {
        ...
        if ((currentState & 1) == 0) {
          if (currentState == 4) {
            if ([VZZXQRCodeFinderPatternFinder foundPatternCross:stateCount]) {
  ...
  [VZZXResultPoint orderBestPatterns:patternInfo];
  return [[VZZXQRCodeFinderPatternInfo alloc] initWithPatternCenters:patternInfo];
}
...
- (BOOL)crossCheckDiagonal:(int)startI centerJ:(int)centerJ maxCount:(int)maxCount originalStateCountTotal:(int)originalStateCountTotal {
  ...
  return
    ...
    [VZZXQRCodeFinderPatternFinder foundPatternCross:stateCount];
}
...
- (float)crossCheckVertical:(int)startI centerJ:(int)centerJ maxCount:(int)maxCount originalStateCountTotal:(int)originalStateCountTotal {
  ...
  return [VZZXQRCodeFinderPatternFinder foundPatternCross:stateCount] ? [self centerFromEnd:stateCount end:i] : NAN;
}
...
- (float)crossCheckHorizontal:(int)startJ centerI:(int)centerI maxCount:(int)maxCount originalStateCountTotal:(int)originalStateCountTotal {
  ...
  return [VZZXQRCodeFinderPatternFinder foundPatternCross:stateCount] ? [self centerFromEnd:stateCount end:j] : NAN;
}
  ...
  if (!isnan(centerI)) {
    ...
    if (!isnan(centerJ) &&
      ...
      for (int index = 0; index < max; index++) {
        VZZXQRCodeFinderPattern *center = self.possibleCenters[index];
      ...
      if (!found) {
        VZZXResultPoint *point = [[VZZXQRCodeFinderPattern alloc] initWithPosX:centerJ posY:centerI estimatedModuleSize:estimatedModuleSize];
...
- (int)findRowSkip {
  ...
  VZZXResultPoint *firstConfirmedCenter = nil;
  for (int i = 0; i < max; i++) {
    VZZXQRCodeFinderPattern *center = self.possibleCenters[i];
    if ([center count] >= VZZX_CENTER_QUORUM) {
 ...
 *         at least VZZX_CENTER_QUORUM times each, and, the estimated module size of the
...
- (BOOL)haveMultiplyConfirmedCenters {
  ...
  int max = (int)[self.possibleCenters count];
  for (int i = 0; i < max; i++) {
    VZZXQRCodeFinderPattern *pattern = self.possibleCenters[i];
    if ([pattern count] >= VZZX_CENTER_QUORUM) {
      confirmedCount++;
...
/**
 * Orders by VZZXFinderPattern count, descending.
 */
NSInteger centerCompare(id center1, id center2, void *context) {
  float average = [(__bridge NSNumber *)context floatValue];

  if ([((VZZXQRCodeFinderPattern *)center2) count] == [((VZZXQRCodeFinderPattern *)center1) count]) {
    float dA = fabsf([((VZZXQRCodeFinderPattern *)center2) estimatedModuleSize] - average);
    float dB = fabsf([((VZZXQRCodeFinderPattern *)center1) estimatedModuleSize] - average);
    return dA < dB ? 1 : dA == dB ? 0 : -1;
  } else {
    return [((VZZXQRCodeFinderPattern *)center2) count] - [((VZZXQRCodeFinderPattern *)center1) count];
  }
}
...
/**
 * @return the 3 best VZZXFinderPatterns from our list of candidates. The "best" are
 *         those that have been detected at least VZZXCENTER_QUORUM times, and whose module
