...
@interface VZZXQRCodeAlignmentPatternFinder ()
...
@property (nonatomic, strong, readonly) VZZXIntArray *crossCheckStateCount;
@property (nonatomic, weak, readonly) id<VZZXResultPointCallback> resultPointCallback;

@end

@implementation VZZXQRCodeAlignmentPatternFinder

- (id)initWithImage:(VZZXBitMatrix *)image startX:(int)startX startY:(int)startY width:(int)width height:(int)height moduleSize:(float)moduleSize resultPointCallback:(id<VZZXResultPointCallback>)resultPointCallback {
  if (self = [super init]) {
    _image = image;
    ...
    _crossCheckStateCount = [[VZZXIntArray alloc] initWithLength:3];
...
- (VZZXQRCodeAlignmentPattern *)findWithError:(NSError **)error {
  ...
  for (int iGen = 0; iGen < self.height; iGen++) {
        ...
        if (currentState == 1) {
          ...
          if (currentState == 2) {
            if ([self foundPatternCross:stateCount]) {
              VZZXQRCodeAlignmentPattern *confirmed = [self handlePossibleCenter:stateCount i:i j:j];
              if (confirmed != nil) {
                return confirmed;
              }
            }
    ...
    if ([self foundPatternCross:stateCount]) {
      VZZXQRCodeAlignmentPattern *confirmed = [self handlePossibleCenter:stateCount i:i j:maxJ];
      if (confirmed != nil) {
        return confirmed;
      }
    }
 ...
 * @return VZZXAlignmentPattern if we have found the same pattern twice, or null if not
 */
- (VZZXQRCodeAlignmentPattern *)handlePossibleCenter:(int *)stateCount i:(int)i j:(int)j {
  int stateCountTotal = stateCount[0] + stateCount[1] + stateCount[2];
  ...
  if (!isnan(centerI)) {
    ...
    for (int index = 0; index < max; index++) {
      VZZXQRCodeAlignmentPattern *center = self.possibleCenters[index];
    ...
    VZZXResultPoint *point = [[VZZXQRCodeAlignmentPattern alloc] initWithPosX:centerJ posY:centerI estimatedModuleSize:estimatedModuleSize];
