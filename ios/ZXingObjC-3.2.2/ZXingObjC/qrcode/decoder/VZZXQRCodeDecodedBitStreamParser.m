 ...
 */
const unichar VZZX_ALPHANUMERIC_CHARS[45] = {
...
const int VZZX_GB2312_SUBSET = 1;

@implementation VZZXQRCodeDecodedBitStreamParser

+ (VZZXDecoderResult *)decode:(VZZXByteArray *)bytes
                    version:(VZZXQRCodeVersion *)version
                    ecLevel:(VZZXQRCodeErrorCorrectionLevel *)ecLevel
                      hints:(VZZXDecodeHints *)hints
                      error:(NSError **)error {
  VZZXBitSource *bits = [[VZZXBitSource alloc] initWithBytes:bytes];
  ...
  VZZXCharacterSetECI *currentCharacterSetECI = nil;
  VZZXQRCodeMode *mode;
  BOOL fc1InEffect = NO;
  ...
  do {
    ...
    if ([bits available] < 4) {
      ...
      mode = [VZZXQRCodeMode terminatorMode];
    } else {
      mode = [VZZXQRCodeMode forBits:[bits readBits:4]]; // mode is encoded by 4 bits
      if (!mode) {
        if (error) *error = VZZXFormatErrorInstance();
        return nil;
      }
    }
    if (![mode isEqual:[VZZXQRCodeMode terminatorMode]]) {
      if ([mode isEqual:[VZZXQRCodeMode fnc1FirstPositionMode]] || [mode isEqual:[VZZXQRCodeMode fnc1SecondPositionMode]]) {
        // We do little with FNC1 except alter the parsed result a bit according to the spec
        fc1InEffect = YES;
      } else if ([mode isEqual:[VZZXQRCodeMode structuredAppendMode]]) {
      ...
      } else if ([mode isEqual:[VZZXQRCodeMode eciMode]]) {
        // Count doesn't apply to ECI
        int value = [self parseECIValue:bits];
        currentCharacterSetECI = [VZZXCharacterSetECI characterSetECIByValue:value];
  ...
  } while (![mode isEqual:[VZZXQRCodeMode terminatorMode]]);

  return [[VZZXDecoderResult alloc] initWithRawBytes:bytes
 ...
 */
+ (BOOL)decodeHanziSegment:(VZZXBitSource *)bits result:(NSMutableString *)result count:(int)count {
...
+ (BOOL)decodeKanjiSegment:(VZZXBitSource *)bits result:(NSMutableString *)result count:(int)count {
  if (count * 13 > bits.available) {
    return NO;
  }
...
+ (BOOL)decodeByteSegment:(VZZXBitSource *)bits result:(NSMutableString *)result count:(int)count currentCharacterSetECI:(VZZXCharacterSetECI *)currentCharacterSetECI byteSegments:(NSMutableArray *)byteSegments hints:(VZZXDecodeHints *)hints {
  if (8 * count > bits.available) {
    return NO;
  }

  VZZXByteArray *readBytes = [[VZZXByteArray alloc] initWithLength:count];
  for (int i = 0; i < count; i++) {
    readBytes.array[i] = (int8_t)[bits readBits:8];
  }
  NSStringEncoding encoding;
  if (currentCharacterSetECI == nil) {
    encoding = [VZZXStringUtils guessEncoding:readBytes hints:hints];
  } else {
...
+ (unichar)toAlphaNumericChar:(int)value {
  ...
  return VZZX_ALPHANUMERIC_CHARS[value];
}

+ (BOOL)decodeAlphanumericSegment:(VZZXBitSource *)bits result:(NSMutableString *)result count:(int)count fc1InEffect:(BOOL)fc1InEffect {
...
+ (BOOL)decodeNumericSegment:(VZZXBitSource *)bits result:(NSMutableString *)result count:(int)count {
...
+ (int)parseECIValue:(VZZXBitSource *)bits {
