...
@interface VZZXQRCodeBitMatrixParser ()

@property (nonatomic, strong, readonly) VZZXBitMatrix *bitMatrix;
@property (nonatomic, assign) BOOL shouldMirror;
@property (nonatomic, strong) VZZXQRCodeFormatInformation *parsedFormatInfo;
@property (nonatomic, strong) VZZXQRCodeVersion *parsedVersion;

@end

@implementation VZZXQRCodeBitMatrixParser

- (id)initWithBitMatrix:(VZZXBitMatrix *)bitMatrix error:(NSError **)error {
...
- (VZZXQRCodeFormatInformation *)readFormatInformationWithError:(NSError **)error {
  ...
  self.parsedFormatInfo = [VZZXQRCodeFormatInformation decodeFormatInformation:formatInfoBits1 maskedFormatInfo2:formatInfoBits2];
  if (self.parsedFormatInfo != nil) {
    return self.parsedFormatInfo;
  }
  if (error) *error = VZZXFormatErrorInstance();
...
- (VZZXQRCodeVersion *)readVersionWithError:(NSError **)error {
  ...
  if (provisionalVersion <= 6) {
    return [VZZXQRCodeVersion versionForNumber:provisionalVersion];
  }
  int versionBits = 0;
  ...
  VZZXQRCodeVersion *theParsedVersion = [VZZXQRCodeVersion decodeVersionInformation:versionBits];
  ...
  theParsedVersion = [VZZXQRCodeVersion decodeVersionInformation:versionBits];
  if (theParsedVersion != nil && theParsedVersion.dimensionForVersion == dimension) {
    self.parsedVersion = theParsedVersion;
    return self.parsedVersion;
  }
...
- (VZZXByteArray *)readCodewordsWithError:(NSError **)error {
  VZZXQRCodeFormatInformation *formatInfo = [self readFormatInformationWithError:error];
  if (!formatInfo) {
    return nil;
  }

  VZZXQRCodeVersion *version = [self readVersionWithError:error];
  if (!version) {
    return nil;
  }
  ...
  VZZXQRCodeDataMask *dataMask = [VZZXQRCodeDataMask forReference:[formatInfo dataMask]];
  int dimension = self.bitMatrix.height;
  [dataMask unmaskBitMatrix:self.bitMatrix dimension:dimension];

  VZZXBitMatrix *functionPattern = [version buildFunctionPattern];

  BOOL readingUp = YES;
  VZZXByteArray *result = [[VZZXByteArray alloc] initWithLength:version.totalCodewords];
  int resultOffset = 0;
  int currentByte = 0;
  int bitsRead = 0;
...
- (void)remask {
  ...
  VZZXQRCodeDataMask *dataMask = [VZZXQRCodeDataMask forReference:self.parsedFormatInfo.dataMask];
  int dimension = self.bitMatrix.height;
  [dataMask unmaskBitMatrix:self.bitMatrix dimension:dimension];
}
