...
@implementation VZZXQRCodeReader

- (id)init {
  if (self = [super init]) {
    _decoder = [[VZZXQRCodeDecoder alloc] init];
  }
...
- (VZZXResult *)decode:(VZZXBinaryBitmap *)image hints:(VZZXDecodeHints *)hints error:(NSError **)error {
  ...
  if (hints != nil && hints.pureBarcode) {
    ...
    VZZXDetectorResult *detectorResult = [[[VZZXQRCodeDetector alloc] initWithImage:matrix] detect:hints error:error];
    if (!detectorResult) {
      return nil;
    }
...
- (float)moduleSize:(VZZXIntArray *)leftTopBlack image:(VZZXBitMatrix *)image {
