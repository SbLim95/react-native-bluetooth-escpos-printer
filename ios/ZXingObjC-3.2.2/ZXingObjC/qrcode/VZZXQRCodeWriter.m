...
const int VZZX_QUIET_ZONE_SIZE = 4;

@implementation VZZXQRCodeWriter
...
- (VZZXBitMatrix *)encode:(NSString *)contents format:(VZZXBarcodeFormat)format width:(int)width height:(int)height hints:(VZZXEncodeHints *)hints error:(NSError **)error {
  ...
  VZZXQRCodeErrorCorrectionLevel *errorCorrectionLevel = [VZZXQRCodeErrorCorrectionLevel errorCorrectionLevelL];
  int quietZone = VZZX_QUIET_ZONE_SIZE;
  if (hints != nil) {
  ...
  VZZXQRCode *code = [VZZXQRCodeEncoder encode:contents ecLevel:errorCorrectionLevel hints:hints error:error];
  return [self renderResult:code width:width height:height quietZone:quietZone];
}

- (VZZXBitMatrix *)renderResult:(VZZXQRCode *)code width:(int)width height:(int)height quietZone:(int)quietZone {
  VZZXByteMatrix *input = code.matrix;
  if (input == nil) {
    return nil;
  }
