...
#import "VZZXErrors.h"
#import "VZZXPDF417HighLevelEncoder.h"

/**
 * code for Text compaction
 */
const int VZZX_PDF417_TEXT_COMPACTION = 0;

/**
 * code for Byte compaction
 */
const int VZZX_PDF417_BYTE_COMPACTION = 1;

/**
 * code for Numeric compaction
 */
const int VZZX_PDF417_NUMERIC_COMPACTION = 2;

/**
 * Text compaction submode Alpha
 */
const int VZZX_PDF417_SUBMODE_ALPHA = 0;

/**
 * Text compaction submode Lower
 */
const int VZZX_PDF417_SUBMODE_LOWER = 1;

/**
 * Text compaction submode Mixed
 */
const int VZZX_PDF417_SUBMODE_MIXED = 2;

/**
 * Text compaction submode Punctuation
 */
const int VZZX_PDF417_SUBMODE_PUNCTUATION = 3;

/**
 * mode latch to Text Compaction mode
 */
const int VZZX_PDF417_LATCH_TO_TEXT = 900;

/**
 * mode latch to Byte Compaction mode (number of characters NOT a multiple of 6)
 */
const int VZZX_PDF417_LATCH_TO_BYTE_PADDED = 901;

/**
 * mode latch to Numeric Compaction mode
 */
const int VZZX_PDF417_LATCH_TO_NUMERIC = 902;

/**
 * mode shift to Byte Compaction mode
 */
const int VZZX_PDF417_SHIFT_TO_BYTE = 913;

/**
 * mode latch to Byte Compaction mode (number of characters a multiple of 6)
 */
const int VZZX_PDF417_LATCH_TO_BYTE = 924;

/**
 * identifier for a user defined Extended Channel Interpretation (ECI)
 */
const int VZZX_PDF417_HIGH_LEVEL_ECI_USER_DEFINED = 925;

/**
 * identifier for a general purpose ECO format
 */
const int VZZX_PDF417_HIGH_LEVEL_ECI_GENERAL_PURPOSE = 926;

/**
 * identifier for an ECI of a character set of code page
 */
const int VZZX_PDF417_HIGH_LEVEL_ECI_CHARSET = 927;

/**
 * Raw code table for text compaction Mixed sub-mode
 */
const int8_t VZZX_PDF417_TEXT_MIXED_RAW[] = {
 ...
 */
const int8_t VZZX_PDF417_TEXT_PUNCTUATION_RAW[] = {
  59, 60, 62, 64, 91, 92, 93, 95, 96, 126, 33, 13, 9, 44, 58,
  10, 45, 46, 36, 47, 34, 124, 42, 40, 41, 63, 123, 125, 39, 0};

const int VZZX_PDF417_MIXED_TABLE_LEN  = 128;
unichar VZZX_PDF417_MIXED_TABLE[VZZX_PDF417_MIXED_TABLE_LEN];

const int VZZX_PDF417_PUNCTUATION_LEN  = 128;
unichar VZZX_PDF417_PUNCTUATION[VZZX_PDF417_PUNCTUATION_LEN];

const NSStringEncoding VZZX_PDF417_DEFAULT_ENCODING = NSISOLatin1StringEncoding;

@implementation VZZXPDF417HighLevelEncoder

+ (void)initialize {
  if ([self class] != [VZZXPDF417HighLevelEncoder class]) return;

  //Construct inverse lookups
  for (int i = 0; i < VZZX_PDF417_MIXED_TABLE_LEN; i++) {
    VZZX_PDF417_MIXED_TABLE[i] = 0xFF;
  }
  for (int8_t i = 0; i < sizeof(VZZX_PDF417_TEXT_MIXED_RAW) / sizeof(int8_t); i++) {
    int8_t b = VZZX_PDF417_TEXT_MIXED_RAW[i];
    if (b > 0) {
      VZZX_PDF417_MIXED_TABLE[b] = i;
    }
  }
  for (int i = 0; i < VZZX_PDF417_PUNCTUATION_LEN; i++) {
    VZZX_PDF417_PUNCTUATION[i] = 0xFF;
  }
  for (int8_t i = 0; i < sizeof(VZZX_PDF417_TEXT_PUNCTUATION_RAW) / sizeof(int8_t); i++) {
    int8_t b = VZZX_PDF417_TEXT_PUNCTUATION_RAW[i];
    if (b > 0) {
      VZZX_PDF417_PUNCTUATION[b] = i;
    }
  }
}

+ (NSString *)encodeHighLevel:(NSString *)msg compaction:(VZZXPDF417Compaction)compaction encoding:(NSStringEncoding)encoding error:(NSError **)error {
  //the codewords 0..928 are encoded as Unicode characters
  NSMutableString *sb = [NSMutableString stringWithCapacity:msg.length];

  if (encoding == 0) {
    encoding = VZZX_PDF417_DEFAULT_ENCODING;
  } else if (VZZX_PDF417_DEFAULT_ENCODING != encoding) {
  ...
  int p = 0;
  int textSubMode = VZZX_PDF417_SUBMODE_ALPHA;

  // User selected encoding mode
  VZZXByteArray *bytes = nil; //Fill later and only if needed
  if (compaction == VZZXPDF417CompactionText) {
    [self encodeText:msg startpos:p count:(int)len buffer:sb initialSubmode:textSubMode];
  } else if (compaction == VZZXPDF417CompactionByte) {
    bytes = [self bytesForMessage:msg encoding:encoding];
    [self encodeBinary:bytes startpos:p count:(int)msg.length startmode:VZZX_PDF417_BYTE_COMPACTION buffer:sb];
  } else if (compaction == VZZXPDF417CompactionNumeric) {
    [sb appendFormat:@"%C", (unichar) VZZX_PDF417_LATCH_TO_NUMERIC];
    [self encodeNumeric:msg startpos:p count:(int)len buffer:sb];
  } else {
    int encodingMode = VZZX_PDF417_TEXT_COMPACTION; //Default mode, see 4.4.2.1
    while (p < len) {
      int n = [self determineConsecutiveDigitCount:msg startpos:p];
      if (n >= 13) {
        [sb appendFormat:@"%C", (unichar) VZZX_PDF417_LATCH_TO_NUMERIC];
        encodingMode = VZZX_PDF417_NUMERIC_COMPACTION;
        textSubMode = VZZX_PDF417_SUBMODE_ALPHA; //Reset after latch
        [self encodeNumeric:msg startpos:p count:n buffer:sb];
        p += n;
      } else {
        int t = [self determineConsecutiveTextCount:msg startpos:p];
        if (t >= 5 || n == len) {
          if (encodingMode != VZZX_PDF417_TEXT_COMPACTION) {
            [sb appendFormat:@"%C", (unichar) VZZX_PDF417_LATCH_TO_TEXT];
            encodingMode = VZZX_PDF417_TEXT_COMPACTION;
            textSubMode = VZZX_PDF417_SUBMODE_ALPHA; //start with submode alpha after latch
          }
          ...
          if (b == 1 && encodingMode == VZZX_PDF417_TEXT_COMPACTION) {
            //Switch for one byte (instead of latch)
            [self encodeBinary:bytes startpos:p count:1 startmode:VZZX_PDF417_TEXT_COMPACTION buffer:sb];
          } else {
            //Mode latch performed by encodeBinary
            [self encodeBinary:bytes startpos:p count:b startmode:encodingMode buffer:sb];
            encodingMode = VZZX_PDF417_BYTE_COMPACTION;
            textSubMode = VZZX_PDF417_SUBMODE_ALPHA; //Reset after latch
          }
          p += b;
        }
...
+ (int)encodeText:(NSString *)msg startpos:(int)startpos count:(int)count buffer:(NSMutableString *)sb initialSubmode:(int)initialSubmode {
  ...
  while (true) {
    ...
    switch (submode) {
      case VZZX_PDF417_SUBMODE_ALPHA:
        if ([self isAlphaUpper:ch]) {
          ...
          if ([self isAlphaLower:ch]) {
            submode = VZZX_PDF417_SUBMODE_LOWER;
            [tmp appendFormat:@"%C", (unichar) 27]; //ll
            continue;
          } else if ([self isMixed:ch]) {
            submode = VZZX_PDF417_SUBMODE_MIXED;
            [tmp appendFormat:@"%C", (unichar) 28]; //ml
            continue;
          } else {
            [tmp appendFormat:@"%C", (unichar) 29]; //ps
            [tmp appendFormat:@"%C", VZZX_PDF417_PUNCTUATION[ch]];
            break;
          }
        }
        break;
      case VZZX_PDF417_SUBMODE_LOWER:
        if ([self isAlphaLower:ch]) {
          if (ch == ' ') {
            [tmp appendFormat:@"%C", (unichar) 26]; //space
          } else {
        ...
        break;
      case VZZX_PDF417_SUBMODE_MIXED:
        if ([self isMixed:ch]) {
          [tmp appendFormat:@"%C", VZZX_PDF417_MIXED_TABLE[ch]]; //as
        } else {
          if ([self isAlphaUpper:ch]) {
            submode = VZZX_PDF417_SUBMODE_ALPHA;
            ...
            if (startpos + idx + 1 < count) {
              ...
              if ([self isPunctuation:next]) {
                submode = VZZX_PDF417_SUBMODE_PUNCTUATION;
        ...
        break;
      default: //VZZX_PDF417_SUBMODE_PUNCTUATION
 ...
 * @param sb        receives the encoded codewords
 */
+ (void)encodeBinary:(VZZXByteArray *)bytes startpos:(int)startpos count:(int)count startmode:(int)startmode buffer:(NSMutableString *)sb {
  if (count == 1 && startmode == VZZX_PDF417_TEXT_COMPACTION) {
    [sb appendFormat:@"%C", (unichar) VZZX_PDF417_SHIFT_TO_BYTE];
  } else {
    BOOL sixpack = ((count % 6) == 0);
    if (sixpack) {
      [sb appendFormat:@"%C", (unichar) VZZX_PDF417_LATCH_TO_BYTE];
    } else {
      [sb appendFormat:@"%C", (unichar) VZZX_PDF417_LATCH_TO_BYTE_PADDED];
    }
  }
...
+ (BOOL)isMixed:(unichar)ch {
  return VZZX_PDF417_MIXED_TABLE[ch] != 0xFF;
}

+ (BOOL)isPunctuation:(unichar)ch {
  return VZZX_PDF417_PUNCTUATION[ch] != 0xFF;
}
 ...
 * @return the requested character count
 */
+ (int)determineConsecutiveBinaryCount:(NSString *)msg bytes:(VZZXByteArray *)bytes startpos:(int)startpos error:(NSError **)error {
  NSUInteger len = msg.length;
  int idx = startpos;
  while (idx < len) {
    char ch = [msg characterAtIndex:idx];
...
+ (BOOL)encodingECI:(int)eci sb:(NSMutableString *)sb error:(NSError **)error {
  if (eci >= 0 && eci < 900) {
    [sb appendFormat:@"%C", (unichar) VZZX_PDF417_HIGH_LEVEL_ECI_CHARSET];
    [sb appendFormat:@"%C", (unichar) eci];
  } else if (eci < 810900) {
    [sb appendFormat:@"%C", (unichar) VZZX_PDF417_HIGH_LEVEL_ECI_GENERAL_PURPOSE];
    [sb appendFormat:@"%C", (unichar) (eci / 900 - 1)];
    [sb appendFormat:@"%C", (unichar) (eci % 900)];
  } else if (eci < 811800) {
    [sb appendFormat:@"%C", (unichar) VZZX_PDF417_HIGH_LEVEL_ECI_USER_DEFINED];
...
+ (VZZXByteArray *)bytesForMessage:(NSString *)msg encoding:(NSStringEncoding)encoding {
  NSData *data = [msg dataUsingEncoding:encoding];
  int8_t *bytes = (int8_t *)[data bytes];
  VZZXByteArray *byteArray = [[VZZXByteArray alloc] initWithLength:(unsigned int)[data length]];
