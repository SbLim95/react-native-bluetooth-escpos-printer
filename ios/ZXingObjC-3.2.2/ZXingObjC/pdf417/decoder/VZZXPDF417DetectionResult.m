...
#import "VZZXPDF417DetectionResultColumn.h"
#import "VZZXPDF417DetectionResultRowIndicatorColumn.h"

const int VZZX_PDF417_ADJUST_ROW_NUMBER_SKIP = 2;

@interface VZZXPDF417DetectionResult ()

@property (nonatomic, strong, readonly) VZZXPDF417BarcodeMetadata *barcodeMetadata;
...
@implementation VZZXPDF417DetectionResult

- (id)initWithBarcodeMetadata:(VZZXPDF417BarcodeMetadata *)barcodeMetadata boundingBox:(VZZXPDF417BoundingBox *)boundingBox {
  self = [super init];
  if (self) {
...
- (NSArray *)detectionResultColumns {
  ...
  int unadjustedCodewordCount = VZZX_PDF417_MAX_CODEWORDS_IN_BARCODE;
...
- (void)adjustIndicatorColumnRowNumbers:(VZZXPDF417DetectionResultColumn *)detectionResultColumn {
  if (detectionResultColumn && (id)detectionResultColumn != [NSNull null]) {
    [(VZZXPDF417DetectionResultRowIndicatorColumn *)detectionResultColumn adjustCompleteIndicatorColumnRowNumbers:self.barcodeMetadata];
  }
}
...
- (void)adjustRowNumbersFromBothRI {
  ...
  NSArray *LRIcodewords = [(VZZXPDF417DetectionResultColumn *)self.detectionResultColumnsInternal[0] codewords];
  NSArray *RRIcodewords = [(VZZXPDF417DetectionResultColumn *)self.detectionResultColumnsInternal[self.barcodeColumnCount + 1] codewords];
  for (int codewordsRow = 0; codewordsRow < [LRIcodewords count]; codewordsRow++) {
    if (LRIcodewords[codewordsRow] != [NSNull null] &&
        ...
        [(VZZXPDF417Codeword *)LRIcodewords[codewordsRow] rowNumber] == [(VZZXPDF417Codeword *)RRIcodewords[codewordsRow] rowNumber]) {
      for (int barcodeColumn = 1; barcodeColumn <= self.barcodeColumnCount; barcodeColumn++) {
        VZZXPDF417Codeword *codeword = [(VZZXPDF417DetectionResultColumn *)self.detectionResultColumnsInternal[barcodeColumn] codewords][codewordsRow];
        if ((id)codeword == [NSNull null]) {
          continue;
        }
        codeword.rowNumber = [(VZZXPDF417Codeword *)LRIcodewords[codewordsRow] rowNumber];
        if (![codeword hasValidRowNumber]) {
          [(VZZXPDF417DetectionResultColumn *)self.detectionResultColumnsInternal[barcodeColumn] codewords][codewordsRow] = [NSNull null];
        }
      }
    }
  }
...
- (int)adjustRowNumbersFromRRI {
  ...
  for (int codewordsRow = 0; codewordsRow < [codewords count]; codewordsRow++) {
    ...
    for (int barcodeColumn = self.barcodeColumnCount + 1; barcodeColumn > 0 && invalidRowCounts < VZZX_PDF417_ADJUST_ROW_NUMBER_SKIP; barcodeColumn--) {
      if (self.detectionResultColumnsInternal[barcodeColumn] != [NSNull null]) {
        VZZXPDF417Codeword *codeword = [self.detectionResultColumnsInternal[barcodeColumn] codewords][codewordsRow];
        if ((id)codeword != [NSNull null]) {
...
- (int)adjustRowNumbersFromLRI {
  ...
  for (int codewordsRow = 0; codewordsRow < [codewords count]; codewordsRow++) {
    ...
    int rowIndicatorRowNumber = [codewords[codewordsRow] rowNumber];
    int invalidRowCounts = 0;
    for (int barcodeColumn = 1; barcodeColumn < self.barcodeColumnCount + 1 && invalidRowCounts < VZZX_PDF417_ADJUST_ROW_NUMBER_SKIP; barcodeColumn++) {
      if (self.detectionResultColumnsInternal[barcodeColumn] != [NSNull null]) {
        VZZXPDF417Codeword *codeword = [self.detectionResultColumnsInternal[barcodeColumn] codewords][codewordsRow];
        if ((id)codeword != [NSNull null]) {
          invalidRowCounts = [self adjustRowNumberIfValid:rowIndicatorRowNumber invalidRowCounts:invalidRowCounts codeword:codeword];
...
- (int)adjustRowNumberIfValid:(int)rowIndicatorRowNumber invalidRowCounts:(int)invalidRowCounts codeword:(VZZXPDF417Codeword *)codeword {
  if (!codeword) {
...
- (void)adjustRowNumbers:(int)barcodeColumn codewordsRow:(int)codewordsRow codewords:(NSArray *)codewords {
  VZZXPDF417Codeword *codeword = codewords[codewordsRow];
  ...
  for (VZZXPDF417Codeword *otherCodeword in otherCodewords) {
 ...
 */
- (BOOL)adjustRowNumber:(VZZXPDF417Codeword *)codeword otherCodeword:(VZZXPDF417Codeword *)otherCodeword {
...
- (void)setDetectionResultColumn:(int)barcodeColumn detectionResultColumn:(VZZXPDF417DetectionResultColumn *)detectionResultColumn {
...
- (VZZXPDF417DetectionResultColumn *)detectionResultColumn:(int)barcodeColumn {
  VZZXPDF417DetectionResultColumn *result = self.detectionResultColumnsInternal[barcodeColumn];
  return (id)result == [NSNull null] ? nil : result;
}

- (NSString *)description {
  VZZXPDF417DetectionResultColumn *rowIndicatorColumn = self.detectionResultColumnsInternal[0];
