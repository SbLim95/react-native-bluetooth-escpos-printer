...
#import "VZZXPDF417BarcodeMetadata.h"
#import "VZZXPDF417BarcodeValue.h"
#import "VZZXPDF417BoundingBox.h"
#import "VZZXPDF417Codeword.h"
#import "VZZXPDF417CodewordDecoder.h"
#import "VZZXPDF417Common.h"
#import "VZZXPDF417DecodedBitStreamParser.h"
#import "VZZXPDF417DetectionResult.h"
#import "VZZXPDF417DetectionResultRowIndicatorColumn.h"
#import "VZZXPDF417ECErrorCorrection.h"
#import "VZZXPDF417ScanningDecoder.h"
#import "VZZXResultPoint.h"

const int VZZX_PDF417_CODEWORD_SKEW_SIZE = 2;

const int VZZX_PDF417_MAX_ERRORS = 3;
const int VZZX_PDF417_MAX_EC_CODEWORDS = 512;
static VZZXPDF417ECErrorCorrection *errorCorrection;

@implementation VZZXPDF417ScanningDecoder

+ (void)initialize {
  if ([self class] != [VZZXPDF417ScanningDecoder class]) return;

  errorCorrection = [[VZZXPDF417ECErrorCorrection alloc] init];
}
...
+ (VZZXDecoderResult *)decode:(VZZXBitMatrix *)image
               imageTopLeft:(VZZXResultPoint *)imageTopLeft
            imageBottomLeft:(VZZXResultPoint *)imageBottomLeft
              imageTopRight:(VZZXResultPoint *)imageTopRight
           imageBottomRight:(VZZXResultPoint *)imageBottomRight
           minCodewordWidth:(int)minCodewordWidth
           maxCodewordWidth:(int)maxCodewordWidth
                      error:(NSError **)error {
  VZZXPDF417BoundingBox *boundingBox = [[VZZXPDF417BoundingBox alloc] initWithImage:image topLeft:imageTopLeft bottomLeft:imageBottomLeft topRight:imageTopRight bottomRight:imageBottomRight];
  VZZXPDF417DetectionResultRowIndicatorColumn *leftRowIndicatorColumn;
  VZZXPDF417DetectionResultRowIndicatorColumn *rightRowIndicatorColumn;
  VZZXPDF417DetectionResult *detectionResult;
  for (int i = 0; i < 2; i++) {
    if (imageTopLeft) {
  ...
  for (int barcodeColumnCount = 1; barcodeColumnCount <= maxBarcodeColumn; barcodeColumnCount++) {
    ...
    VZZXPDF417DetectionResultColumn *detectionResultColumn;
    if (barcodeColumn == 0 || barcodeColumn == maxBarcodeColumn) {
      detectionResultColumn = [[VZZXPDF417DetectionResultRowIndicatorColumn alloc] initWithBoundingBox:boundingBox isLeft:barcodeColumn == 0];
    } else {
      detectionResultColumn = [[VZZXPDF417DetectionResultColumn alloc] initWithBoundingBox:boundingBox];
    }
    ...
    for (int imageRow = boundingBox.minY; imageRow <= boundingBox.maxY; imageRow++) {
      ...
      VZZXPDF417Codeword *codeword = [self detectCodeword:image minColumn:boundingBox.minX maxColumn:boundingBox.maxX leftToRight:leftToRight
...
+ (VZZXPDF417DetectionResult *)merge:(VZZXPDF417DetectionResultRowIndicatorColumn *)leftRowIndicatorColumn
           rightRowIndicatorColumn:(VZZXPDF417DetectionResultRowIndicatorColumn *)rightRowIndicatorColumn
                             error:(NSError **)error {
  if (!leftRowIndicatorColumn && !rightRowIndicatorColumn) {
    return nil;
  }
  VZZXPDF417BarcodeMetadata *barcodeMetadata = [self barcodeMetadata:leftRowIndicatorColumn rightRowIndicatorColumn:rightRowIndicatorColumn];
  if (!barcodeMetadata) {
    return nil;
  }
  VZZXPDF417BoundingBox *leftBoundingBox, *rightBoundingBox;
  ...
  VZZXPDF417BoundingBox *boundingBox = [VZZXPDF417BoundingBox mergeLeftBox:leftBoundingBox rightBox:rightBoundingBox];
  if (!boundingBox) {
    if (error) *error = VZZXNotFoundErrorInstance();
    return nil;
  }
  return [[VZZXPDF417DetectionResult alloc] initWithBarcodeMetadata:barcodeMetadata boundingBox:boundingBox];
}

+ (BOOL)adjustBoundingBox:(VZZXPDF417BoundingBox **)boundingBox
       rowIndicatorColumn:(VZZXPDF417DetectionResultRowIndicatorColumn *)rowIndicatorColumn
                    error:(NSError **)error {
  if (!rowIndicatorColumn) {
    *boundingBox = nil;
    return YES;
  }
  VZZXIntArray *rowHeights;
...
+ (VZZXPDF417BarcodeMetadata *)barcodeMetadata:(VZZXPDF417DetectionResultRowIndicatorColumn *)leftRowIndicatorColumn
                     rightRowIndicatorColumn:(VZZXPDF417DetectionResultRowIndicatorColumn *)rightRowIndicatorColumn {
  VZZXPDF417BarcodeMetadata *leftBarcodeMetadata;
  ...
  VZZXPDF417BarcodeMetadata *rightBarcodeMetadata;
...
+ (VZZXPDF417DetectionResultRowIndicatorColumn *)rowIndicatorColumn:(VZZXBitMatrix *)image
                                                      boundingBox:(VZZXPDF417BoundingBox *)boundingBox
                                                       startPoint:(VZZXResultPoint *)startPoint
                                                      leftToRight:(BOOL)leftToRight
                                                 minCodewordWidth:(int)minCodewordWidth
                                                 maxCodewordWidth:(int)maxCodewordWidth {
  VZZXPDF417DetectionResultRowIndicatorColumn *rowIndicatorColumn = [[VZZXPDF417DetectionResultRowIndicatorColumn alloc] initWithBoundingBox:boundingBox
                                                                                                                                  isLeft:leftToRight];
  for (int i = 0; i < 2; i++) {
    ...
    for (int imageRow = (int) startPoint.y; imageRow <= boundingBox.maxY &&
      ...
      VZZXPDF417Codeword *codeword = [self detectCodeword:image minColumn:0 maxColumn:image.width leftToRight:leftToRight startColumn:startColumn imageRow:imageRow
                                       minCodewordWidth:minCodewordWidth maxCodewordWidth:maxCodewordWidth];
      if (codeword) {
...
+ (BOOL)adjustCodewordCount:(VZZXPDF417DetectionResult *)detectionResult barcodeMatrix:(NSArray *)barcodeMatrix {
  VZZXIntArray *numberOfCodewords = [(VZZXPDF417BarcodeValue *)barcodeMatrix[0][1] value];
  ...
  if (numberOfCodewords.length == 0) {
    if (calculatedNumberOfCodewords < 1 || calculatedNumberOfCodewords > VZZX_PDF417_MAX_CODEWORDS_IN_BARCODE) {
      return NO;
    }
    [(VZZXPDF417BarcodeValue *)barcodeMatrix[0][1] setValue:calculatedNumberOfCodewords];
...
+ (VZZXDecoderResult *)createDecoderResult:(VZZXPDF417DetectionResult *)detectionResult error:(NSError **)error {
  ...
  VZZXIntArray *codewords = [[VZZXIntArray alloc] initWithLength:detectionResult.barcodeRowCount * detectionResult.barcodeColumnCount];
  ...
  for (int row = 0; row < detectionResult.barcodeRowCount; row++) {
    for (int column = 0; column < detectionResult.barcodeColumnCount; column++) {
      VZZXIntArray *values = [(VZZXPDF417BarcodeValue *)barcodeMatrix[row][column + 1] value];
  ...
  return [self createDecoderResultFromAmbiguousValues:detectionResult.barcodeECLevel
                                            codewords:codewords
                                         erasureArray:[VZZXPDF417Common toIntArray:erasures]
                                     ambiguousIndexes:[VZZXPDF417Common toIntArray:ambiguousIndexesList]
 ...
 */
+ (VZZXDecoderResult *)createDecoderResultFromAmbiguousValues:(int)ecLevel
                                                  codewords:(VZZXIntArray *)codewords
                                               erasureArray:(VZZXIntArray *)erasureArray
                                           ambiguousIndexes:(VZZXIntArray *)ambiguousIndexes
                                       ambiguousIndexValues:(NSArray *)ambiguousIndexValues
                                                      error:(NSError **)error {
  VZZXIntArray *ambiguousIndexCount = [[VZZXIntArray alloc] initWithLength:ambiguousIndexes.length];

  int tries = 100;
  while (tries-- > 0) {
    for (int i = 0; i < ambiguousIndexCount.length; i++) {
      VZZXIntArray *a = ambiguousIndexValues[i];
      codewords.array[ambiguousIndexes.array[i]] = a.array[(ambiguousIndexCount.array[i] + 1) % [(VZZXIntArray *)ambiguousIndexValues[i] length]];
    }
    NSError *e;
    VZZXDecoderResult *result = [self decodeCodewords:codewords ecLevel:ecLevel erasures:erasureArray error:&e];
    if (result) {
      return result;
    } else if (e.code != VZZXChecksumError) {
      if (error) *error = e;
      return nil;
    }
    ...
    for (int i = 0; i < ambiguousIndexCount.length; i++) {
      if (ambiguousIndexCount.array[i] < [(VZZXIntArray *)ambiguousIndexValues[i] length] - 1) {
...
+ (NSArray *)createBarcodeMatrix:(VZZXPDF417DetectionResult *)detectionResult {
  ...
  for (int row = 0; row < detectionResult.barcodeRowCount; row++) {
    ...
    for (int column = 0; column < detectionResult.barcodeColumnCount + 2; column++) {
      barcodeMatrix[row][column] = [[VZZXPDF417BarcodeValue alloc] init];
    }
  }

  int column = 0;
  for (VZZXPDF417DetectionResultColumn *detectionResultColumn in [detectionResult detectionResultColumns]) {
    if ((id)detectionResultColumn != [NSNull null]) {
      for (VZZXPDF417Codeword *codeword in detectionResultColumn.codewords) {
        if ((id)codeword != [NSNull null]) {
          ...
          if (rowNumber >= 0) {
            ...
            [(VZZXPDF417BarcodeValue *)barcodeMatrix[rowNumber][column] setValue:codeword.value];
          }
        }
      }
    }
...
+ (BOOL)isValidBarcodeColumn:(VZZXPDF417DetectionResult *)detectionResult barcodeColumn:(int)barcodeColumn {
  return barcodeColumn >= 0 && barcodeColumn <= detectionResult.barcodeColumnCount + 1;
}

+ (int)startColumn:(VZZXPDF417DetectionResult *)detectionResult
     barcodeColumn:(int)barcodeColumn
          imageRow:(int)imageRow
       leftToRight:(BOOL)leftToRight {
  int offset = leftToRight ? 1 : -1;
  VZZXPDF417Codeword *codeword;
  ...
  while ([self isValidBarcodeColumn:detectionResult barcodeColumn:barcodeColumn - offset]) {
    ...
    for (VZZXPDF417Codeword *previousRowCodeword in [detectionResult detectionResultColumn:barcodeColumn].codewords) {
...
+ (VZZXPDF417Codeword *)detectCodeword:(VZZXBitMatrix *)image
  ...
  int endColumn;
  int codewordBitCount = [VZZXPDF417Common bitCountSum:moduleBitCount];
  if (leftToRight) {
  ...
  int decodedValue = [VZZXPDF417CodewordDecoder decodedValue:moduleBitCount];
  int codeword = [VZZXPDF417Common codeword:decodedValue];
  if (codeword == -1) {
    return nil;
  }
  return [[VZZXPDF417Codeword alloc] initWithStartX:startColumn endX:endColumn bucket:[self codewordBucketNumber:decodedValue] value:codeword];
}

+ (NSMutableArray *)moduleBitCount:(VZZXBitMatrix *)image
                         minColumn:(int)minColumn
                         maxColumn:(int)maxColumn
                       leftToRight:(BOOL)leftToRight
                       startColumn:(int)startColumn
...
+ (int)adjustCodewordStartColumn:(VZZXBitMatrix *)image
                       minColumn:(int)minColumn
                       maxColumn:(int)maxColumn
                     leftToRight:(BOOL)leftToRight
  ...
  for (int i = 0; i < 2; i++) {
    while (((leftToRight && correctedStartColumn >= minColumn) || (!leftToRight && correctedStartColumn < maxColumn)) &&
      ...
      if (abs(codewordStartColumn - correctedStartColumn) > VZZX_PDF417_CODEWORD_SKEW_SIZE) {
...
+ (BOOL)checkCodewordSkew:(int)codewordSize minCodewordWidth:(int)minCodewordWidth maxCodewordWidth:(int)maxCodewordWidth {
  return minCodewordWidth - VZZX_PDF417_CODEWORD_SKEW_SIZE <= codewordSize &&
      codewordSize <= maxCodewordWidth + VZZX_PDF417_CODEWORD_SKEW_SIZE;
}

+ (VZZXDecoderResult *)decodeCodewords:(VZZXIntArray *)codewords ecLevel:(int)ecLevel erasures:(VZZXIntArray *)erasures error:(NSError **)error {
  ...
  VZZXDecoderResult *decoderResult = [VZZXPDF417DecodedBitStreamParser decode:codewords ecLevel:[@(ecLevel) stringValue] error:error];
  if (!decoderResult) {
    return nil;
  }
 ...
 */
+ (int)correctErrors:(VZZXIntArray *)codewords erasures:(VZZXIntArray *)erasures numECCodewords:(int)numECCodewords {
  if (erasures &&
      (erasures.length > numECCodewords / 2 + VZZX_PDF417_MAX_ERRORS ||
       numECCodewords < 0 ||
       numECCodewords > VZZX_PDF417_MAX_EC_CODEWORDS)) {
 ...
 */
+ (BOOL)verifyCodewordCount:(VZZXIntArray *)codewords numECCodewords:(int)numECCodewords {
...
- (NSString *)description:(NSArray *)barcodeMatrix {
  ...
  for (int row = 0; row < [barcodeMatrix count]; row++) {
    ...
    for (int column = 0; column < [(NSArray *)barcodeMatrix[row] count]; column++) {
      VZZXPDF417BarcodeValue *barcodeValue = barcodeMatrix[row][column];
