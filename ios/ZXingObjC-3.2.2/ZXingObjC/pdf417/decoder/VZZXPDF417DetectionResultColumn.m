...
const int VZZX_PDF417_MAX_NEARBY_DISTANCE = 5;

@implementation VZZXPDF417DetectionResultColumn

- (id)initWithBoundingBox:(VZZXPDF417BoundingBox *)boundingBox {
  self = [super init];
  if (self) {
    _boundingBox = [[VZZXPDF417BoundingBox alloc] initWithBoundingBox:boundingBox];
...
- (VZZXPDF417Codeword *)codewordNearby:(int)imageRow {
  VZZXPDF417Codeword *codeword = [self codeword:imageRow];
  if (codeword) {
    return codeword;
  }
  for (int i = 1; i < VZZX_PDF417_MAX_NEARBY_DISTANCE; i++) {
...
- (void)setCodeword:(int)imageRow codeword:(VZZXPDF417Codeword *)codeword {
  _codewords[[self imageRowToCodewordIndex:imageRow]] = codeword;
}

- (VZZXPDF417Codeword *)codeword:(int)imageRow {
