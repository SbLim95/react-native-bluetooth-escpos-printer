...
@interface VZZXModulusPoly ()

@property (nonatomic, strong, readonly) VZZXIntArray *coefficients;
@property (nonatomic, weak, readonly) VZZXModulusGF *field;

@end

@implementation VZZXModulusPoly
...
- (VZZXModulusPoly *)add:(VZZXModulusPoly *)other {
  if (![self.field isEqual:other.field]) {
    [NSException raise:NSInvalidArgumentException format:@"VZZXModulusPolys do not have same VZZXModulusGF field"];
  }
  if (self.zero) {
    return other;
  }
  ...
  return [[VZZXModulusPoly alloc] initWithField:self.field coefficients:sumDiff];
}
...
- (VZZXModulusPoly *)multiply:(VZZXModulusPoly *)other {
  ...
  return [[VZZXModulusPoly alloc] initWithField:self.field coefficients:product];
}

- (VZZXModulusPoly *)negative {
  int size = self.coefficients.length;
  VZZXIntArray *negativeCoefficients = [[VZZXIntArray alloc] initWithLength:size];
  for (int i = 0; i < size; i++) {
    negativeCoefficients.array[i] = [self.field subtract:0 b:self.coefficients.array[i]];
  }
  return [[VZZXModulusPoly alloc] initWithField:self.field coefficients:negativeCoefficients];
}
...
- (NSArray *)divide:(VZZXModulusPoly *)other {
  ...
  VZZXModulusPoly *quotient = self.field.zero;
  VZZXModulusPoly *remainder = self;
  ...
  while ([remainder degree] >= other.degree && !remainder.zero) {
    ...
    VZZXModulusPoly *term = [other multiplyByMonomial:degreeDifference coefficient:scale];
    VZZXModulusPoly *iterationQuotient = [self.field buildMonomial:degreeDifference coefficient:scale];
