...
@implementation VZZXPDF417DetectionResultRowIndicatorColumn

- (id)initWithBoundingBox:(VZZXPDF417BoundingBox *)boundingBox isLeft:(BOOL)isLeft {
...
- (void)setRowNumbers {
  for (VZZXPDF417Codeword *codeword in [self codewords]) {
...
- (int)adjustCompleteIndicatorColumnRowNumbers:(VZZXPDF417BarcodeMetadata *)barcodeMetadata {
  [self setRowNumbers];
  [self removeIncorrectCodewords:barcodeMetadata];
  VZZXResultPoint *top = self.isLeft ? self.boundingBox.topLeft : self.boundingBox.topRight;
  VZZXResultPoint *bottom = self.isLeft ? self.boundingBox.bottomLeft : self.boundingBox.bottomRight;
  ...
  for (int codewordsRow = firstRow; codewordsRow < lastRow; codewordsRow++) {
    ...
    VZZXPDF417Codeword *codeword = self.codewords[codewordsRow];
...
- (BOOL)getRowHeights:(VZZXIntArray **)rowHeights {
  VZZXPDF417BarcodeMetadata *barcodeMetadata = [self barcodeMetadata];
  if (!barcodeMetadata) {
    *rowHeights = nil;
    return YES;
  }
  [self adjustIncompleteIndicatorColumnRowNumbers:barcodeMetadata];
  VZZXIntArray *result = [[VZZXIntArray alloc] initWithLength:barcodeMetadata.rowCount];
  for (VZZXPDF417Codeword *codeword in [self codewords]) {
    if ((id)codeword != [NSNull null]) {
...
// finding row numbers for other columns easier
// use row height count to make detection of invalid row numbers more reliable
- (int)adjustIncompleteIndicatorColumnRowNumbers:(VZZXPDF417BarcodeMetadata *)barcodeMetadata {
  VZZXResultPoint *top = self.isLeft ? self.boundingBox.topLeft : self.boundingBox.topRight;
  VZZXResultPoint *bottom = self.isLeft ? self.boundingBox.bottomLeft : self.boundingBox.bottomRight;
  int firstRow = [self imageRowToCodewordIndex:(int) top.y];
  int lastRow = [self imageRowToCodewordIndex:(int) bottom.y];
...
- (VZZXPDF417BarcodeMetadata *)barcodeMetadata {
  VZZXPDF417BarcodeValue *barcodeColumnCount = [[VZZXPDF417BarcodeValue alloc] init];
  VZZXPDF417BarcodeValue *barcodeRowCountUpperPart = [[VZZXPDF417BarcodeValue alloc] init];
  VZZXPDF417BarcodeValue *barcodeRowCountLowerPart = [[VZZXPDF417BarcodeValue alloc] init];
  VZZXPDF417BarcodeValue *barcodeECLevel = [[VZZXPDF417BarcodeValue alloc] init];
  for (VZZXPDF417Codeword *codeword in self.codewords) {
    if ((id)codeword == [NSNull null]) {
      continue;
    }
  ...
  if (([barcodeColumnCount value].length == 0) ||
      ...
      [barcodeRowCountUpperPart value].array[0] + [barcodeRowCountLowerPart value].array[0] < VZZX_PDF417_MIN_ROWS_IN_BARCODE ||
      [barcodeRowCountUpperPart value].array[0] + [barcodeRowCountLowerPart value].array[0] > VZZX_PDF417_MAX_ROWS_IN_BARCODE) {
    return nil;
  }
  VZZXPDF417BarcodeMetadata *barcodeMetadata = [[VZZXPDF417BarcodeMetadata alloc] initWithColumnCount:[barcodeColumnCount value].array[0]
...
- (void)removeIncorrectCodewords:(VZZXPDF417BarcodeMetadata *)barcodeMetadata {
  // Remove codewords which do not match the metadata
  // TODO Maybe we should keep the incorrect codewords for the start and end positions?
  for (int codewordRow = 0; codewordRow < [self.codewords count]; codewordRow++) {
    VZZXPDF417Codeword *codeword = self.codewords[codewordRow];
