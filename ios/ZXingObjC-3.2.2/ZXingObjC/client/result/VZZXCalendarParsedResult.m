...
static NSRegularExpression *VZZX_DATE_TIME = nil;
static NSRegularExpression *VZZX_RFC2445_DURATION = nil;

const long VZZX_RFC2445_DURATION_FIELD_UNITS[] = {
...
@implementation VZZXCalendarParsedResult

+ (void)initialize {
  if ([self class] != [VZZXCalendarParsedResult class]) return;

  VZZX_DATE_TIME = [[NSRegularExpression alloc] initWithPattern:@"[0-9]{8}(T[0-9]{6}Z?)?"
                                                      options:0
                                                        error:nil];

  VZZX_RFC2445_DURATION = [[NSRegularExpression alloc] initWithPattern:@"P(?:(\\d+)W)?(?:(\\d+)D)?(?:T(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)S)?)?"
...
- (NSString *)displayResult {
  ...
  [VZZXParsedResult maybeAppend:self.summary result:result];
  [VZZXParsedResult maybeAppend:[self format:self.startAllDay date:self.start] result:result];
  [VZZXParsedResult maybeAppend:[self format:self.endAllDay date:self.end] result:result];
  [VZZXParsedResult maybeAppend:self.location result:result];
  [VZZXParsedResult maybeAppend:self.organizer result:result];
  [VZZXParsedResult maybeAppendArray:self.attendees result:result];
  [VZZXParsedResult maybeAppend:self.description result:result];
  return result;
}
...
- (NSDate *)parseDate:(NSString *)when {
  NSArray *matches = [VZZX_DATE_TIME matchesInString:when options:0 range:NSMakeRange(0, when.length)];
  if (matches.count == 0) {
...
- (long)parseDurationMS:(NSString *)durationString {
  ...
  NSArray *m = [VZZX_RFC2445_DURATION matchesInString:durationString options:0 range:NSMakeRange(0, durationString.length)];
  if (m.count == 0) {
    return -1;
  }
  long durationMS = 0;
  NSTextCheckingResult *match = m[0];
  for (int i = 0; i < sizeof(VZZX_RFC2445_DURATION_FIELD_UNITS) / sizeof(long); i++) {
    if ([match rangeAtIndex:i + 1].location != NSNotFound) {
      ...
      if (fieldValue != nil) {
        durationMS += VZZX_RFC2445_DURATION_FIELD_UNITS[i] * [fieldValue intValue];
      }
    }
  }
  return durationMS;
