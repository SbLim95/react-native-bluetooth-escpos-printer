...
static NSRegularExpression *VZZX_GEO_URL_PATTERN = nil;

@implementation VZZXGeoResultParser

+ (void)initialize {
  if ([self class] != [VZZXGeoResultParser class]) return;

  VZZX_GEO_URL_PATTERN = [[NSRegularExpression alloc] initWithPattern:@"geo:([\\-0-9.]+),([\\-0-9.]+)(?:,([\\-0-9.]+))?(?:\\?(.*))?"
...
- (VZZXParsedResult *)parse:(VZZXResult *)result {
  ...
  NSArray *matches = [VZZX_GEO_URL_PATTERN matchesInString:rawText options:0 range:NSMakeRange(0, rawText.length)];
  if (matches.count == 0) {
    return nil;
  }
  ...
  return [VZZXGeoParsedResult geoParsedResultWithLatitude:latitude longitude:longitude altitude:altitude query:query];
}
