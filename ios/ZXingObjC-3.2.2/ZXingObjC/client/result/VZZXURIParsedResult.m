...
static NSRegularExpression *VZZX_USER_IN_HOST = nil;

@implementation VZZXURIParsedResult

+ (void)initialize {
  if ([self class] != [VZZXURIParsedResult class]) return;

  VZZX_USER_IN_HOST = [[NSRegularExpression alloc] initWithPattern:@":/*([^/@]+)@[^/]+" options:0 error:nil];
}
...
- (BOOL)possiblyMaliciousURI {
  return [VZZX_USER_IN_HOST numberOfMatchesInString:self.uri options:0 range:NSMakeRange(0, self.uri.length)] > 0;
}

- (NSString *)displayResult {
  NSMutableString *result = [NSMutableString stringWithCapacity:30];
  [VZZXParsedResult maybeAppend:self.title result:result];
  [VZZXParsedResult maybeAppend:self.uri result:result];
  return result;
}
...
- (BOOL)isColonFollowedByPortNumber:(NSString *)uri protocolEnd:(int)protocolEnd {
  ...
  return [VZZXResultParser isSubstringOfDigits:uri offset:start length:(int)nextSlash - start];
}
