...
static NSRegularExpression *VZZX_ATEXT_ALPHANUMERIC = nil;

@implementation VZZXEmailDoCoMoResultParser

+ (void)initialize {
  if ([self class] != [VZZXEmailDoCoMoResultParser class]) return;

  VZZX_ATEXT_ALPHANUMERIC = [[NSRegularExpression alloc] initWithPattern:@"^[a-zA-Z0-9@.!#$%&'*+\\-/=?^_`{|}~]+$"
                                                               options:0 error:nil];
}
...
- (VZZXParsedResult *)parse:(VZZXResult *)result {
  ...
  return [[VZZXEmailAddressParsedResult alloc] initWithTos:tos ccs:nil bccs:nil subject:subject body:body];
}

+ (BOOL)isBasicallyValidEmailAddress:(NSString *)email {
  return email != nil && [VZZX_ATEXT_ALPHANUMERIC numberOfMatchesInString:email options:0 range:NSMakeRange(0, email.length)] > 0 && [email rangeOfString:@"@"].location != NSNotFound;
}
