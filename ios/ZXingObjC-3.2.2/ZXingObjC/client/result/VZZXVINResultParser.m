...
static NSRegularExpression *VZZX_IOQ = nil;
static NSRegularExpression *VZZX_AZ09 = nil;

@implementation VZZXVINResultParser

+ (void)initialize {
  if ([self class] != [VZZXVINResultParser class]) return;

  VZZX_IOQ = [[NSRegularExpression alloc] initWithPattern:@"[IOQ]" options:0 error:nil];
  VZZX_AZ09 = [[NSRegularExpression alloc] initWithPattern:@"[A-Z0-9]{17}" options:0 error:nil];
}

- (VZZXVINParsedResult *)parse:(VZZXResult *)result {
  if (result.barcodeFormat != kBarcodeFormatCode39) {
    return nil;
  }
  NSString *rawText = result.text;
  rawText = [[VZZX_IOQ stringByReplacingMatchesInString:rawText options:0 range:NSMakeRange(0, rawText.length) withTemplate:@""] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
  if ([VZZX_AZ09 matchesInString:rawText options:0 range:NSMakeRange(0, rawText.length)] == 0) {
    return nil;
  }
  ...
  return [[VZZXVINParsedResult alloc] initWithVIN:rawText
