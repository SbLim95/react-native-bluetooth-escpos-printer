...
static NSRegularExpression *VZZX_BEGIN_VCARD = nil;
static NSRegularExpression *VZZX_VCARD_LIKE_DATE = nil;
static NSRegularExpression *VZZX_CR_LF_SPACE_TAB = nil;
static NSRegularExpression *VZZX_NEWLINE_ESCAPE = nil;
static NSRegularExpression *VZZX_VCARD_ESCAPES = nil;
static NSString *VZZX_EQUALS = @"=";
static NSString *VZZX_SEMICOLON = @";";
static NSRegularExpression *VZZX_UNESCAPED_SEMICOLONS = nil;
static NSCharacterSet *VZZX_COMMA = nil;
static NSCharacterSet *VZZX_SEMICOLON_OR_COMMA = nil;

@implementation VZZXVCardResultParser

+ (void)initialize {
  if ([self class] != [VZZXVCardResultParser class]) return;

  VZZX_BEGIN_VCARD = [[NSRegularExpression alloc] initWithPattern:@"BEGIN:VCARD" options:NSRegularExpressionCaseInsensitive error:nil];
  VZZX_VCARD_LIKE_DATE = [[NSRegularExpression alloc] initWithPattern:@"\\d{4}-?\\d{2}-?\\d{2}" options:0 error:nil];
  VZZX_CR_LF_SPACE_TAB = [[NSRegularExpression alloc] initWithPattern:@"\r\n[ \t]" options:0 error:nil];
  VZZX_NEWLINE_ESCAPE = [[NSRegularExpression alloc] initWithPattern:@"\\\\[nN]" options:0 error:nil];
  VZZX_VCARD_ESCAPES = [[NSRegularExpression alloc] initWithPattern:@"\\\\([,;\\\\])" options:0 error:nil];
  VZZX_UNESCAPED_SEMICOLONS = [[NSRegularExpression alloc] initWithPattern:@"(?<!\\\\);+" options:0 error:nil];
  VZZX_COMMA = [NSCharacterSet characterSetWithCharactersInString:@","];
  VZZX_SEMICOLON_OR_COMMA = [NSCharacterSet characterSetWithCharactersInString:@";,"];
}
...
- (VZZXParsedResult *)parse:(VZZXResult *)result {
  ...
  if ([VZZX_BEGIN_VCARD numberOfMatchesInString:rawText options:0 range:NSMakeRange(0, rawText.length)] == 0) {
    return nil;
  }
  ...
  NSArray *nicknames = nicknameString == nil ? nil : [nicknameString[0] componentsSeparatedByCharactersInSet:VZZX_COMMA];
  ...
  NSArray *geo = geoString == nil ? nil : [geoString[0] componentsSeparatedByCharactersInSet:VZZX_SEMICOLON_OR_COMMA];
  if (geo != nil && geo.count != 2) {
    geo = nil;
  }
  return [VZZXAddressBookParsedResult addressBookParsedResultWithNames:[self toPrimaryValues:names]
                                                           nicknames:nicknames
                                                       pronunciation:nil
...
+ (NSMutableArray *)matchVCardPrefixedField:(NSString *)prefix rawText:(NSString *)rawText trim:(BOOL)trim parseFieldDivider:(BOOL)parseFieldDivider {
  ...
  while (i < max) {
    ...
    if (metadataString != nil) {
      for (NSString *metadatum in [metadataString componentsSeparatedByString:VZZX_SEMICOLON]) {
        if (metadata == nil) {
          metadata = [NSMutableArray array];
        }
        [metadata addObject:metadatum];
        NSUInteger equals = [metadatum rangeOfString:VZZX_EQUALS].location;
    ...
    if (i == NSNotFound) {
      ...
      if (quotedPrintable) {
        ...
        if (parseFieldDivider) {
          element = [[VZZX_UNESCAPED_SEMICOLONS stringByReplacingMatchesInString:element options:0 range:NSMakeRange(0, element.length) withTemplate:@"\n"]
        ...
        element = [VZZX_CR_LF_SPACE_TAB stringByReplacingMatchesInString:element options:0 range:NSMakeRange(0, element.length) withTemplate:@""];
        element = [VZZX_NEWLINE_ESCAPE stringByReplacingMatchesInString:element options:0 range:NSMakeRange(0, element.length) withTemplate:@"\n"];
        element = [VZZX_VCARD_ESCAPES stringByReplacingMatchesInString:element options:0 range:NSMakeRange(0, element.length) withTemplate:@"$1"];
      }
      if (metadata == nil) {
...
- (BOOL)isLikeVCardDate:(NSString *)value {
  return value == nil || [VZZX_VCARD_LIKE_DATE numberOfMatchesInString:value options:0 range:NSMakeRange(0, value.length)] > 0;
}
