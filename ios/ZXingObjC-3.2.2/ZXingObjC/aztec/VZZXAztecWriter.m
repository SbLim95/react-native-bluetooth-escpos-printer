...
const NSStringEncoding VZZX_AZTEC_DEFAULT_ENCODING = NSISOLatin1StringEncoding;

@implementation VZZXAztecWriter
...
- (VZZXBitMatrix *)encode:(NSString *)contents format:(VZZXBarcodeFormat)format width:(int)width height:(int)height hints:(VZZXEncodeHints *)hints error:(NSError **)error {
  ...
  return [self encode:contents
               format:format
                width:width
               height:height
             encoding:encoding == 0 ? VZZX_AZTEC_DEFAULT_ENCODING : encoding
           eccPercent:eccPercent == nil ? VZZX_AZTEC_DEFAULT_EC_PERCENT : [eccPercent intValue]
               layers:layers == nil ? VZZX_AZTEC_DEFAULT_LAYERS : [layers intValue]];
}

- (VZZXBitMatrix *)encode:(NSString *)contents format:(VZZXBarcodeFormat)format width:(int)width
  ...
  VZZXByteArray *bytes = [[VZZXByteArray alloc] initWithLength:(unsigned int)[data length]];
  memcpy(bytes.array, [data bytes], bytes.length * sizeof(int8_t));
  VZZXAztecCode *aztec = [VZZXAztecEncoder encode:bytes minECCPercent:eccPercent userSpecifiedLayers:layers];
  return [self renderResult:aztec width:width height:height];
}

- (VZZXBitMatrix *)renderResult:(VZZXAztecCode *)aztec width:(int)width height:(int)height {
  VZZXBitMatrix *input = aztec.matrix;
  if (!input) {
    return nil;
  }
