...
const int VZZX_AZTEC_DEFAULT_EC_PERCENT = 33; // default minimal percentage of error check words
const int VZZX_AZTEC_DEFAULT_LAYERS = 0;
const int VZZX_AZTEC_MAX_NB_BITS = 32;
const int VZZX_AZTEC_MAX_NB_BITS_COMPACT = 4;

const int VZZX_AZTEC_WORD_SIZE[] = {
  4, 6, 6, 8, 8, 8, 8, 8, 8, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10,
  12, 12, 12, 12, 12, 12, 12, 12, 12, 12
};

@implementation VZZXAztecEncoder

+ (VZZXAztecCode *)encode:(VZZXByteArray *)data {
  return [self encode:data minECCPercent:VZZX_AZTEC_DEFAULT_EC_PERCENT userSpecifiedLayers:VZZX_AZTEC_DEFAULT_LAYERS];
}

+ (VZZXAztecCode *)encode:(VZZXByteArray *)data minECCPercent:(int)minECCPercent userSpecifiedLayers:(int)userSpecifiedLayers {
  // High-level encode
  VZZXBitArray *bits = [[[VZZXAztecHighLevelEncoder alloc] initWithText:data] encode];
  ...
  int wordSize = VZZX_AZTEC_WORD_SIZE[0];
  VZZXBitArray *stuffedBits;
  if (userSpecifiedLayers != VZZX_AZTEC_DEFAULT_LAYERS) {
    compact = userSpecifiedLayers < 0;
    layers = abs(userSpecifiedLayers);
    if (layers > (compact ? VZZX_AZTEC_MAX_NB_BITS_COMPACT : VZZX_AZTEC_MAX_NB_BITS)) {
    ...
    wordSize = VZZX_AZTEC_WORD_SIZE[layers];
    ...
    for (int i = 0; ; i++) {
      ...
      if (wordSize != VZZX_AZTEC_WORD_SIZE[layers]) {
        wordSize = VZZX_AZTEC_WORD_SIZE[layers];
        stuffedBits = [self stuffBits:bits wordSize:wordSize];
      }
      int usableBitsInLayers = totalBitsInLayer - (totalBitsInLayer % wordSize);
  ...
  VZZXBitArray *messageBits = [self generateCheckWords:stuffedBits totalBits:totalBitsInLayer wordSize:wordSize];

  // generate check words
  int messageSizeInWords = stuffedBits.size / wordSize;
  VZZXBitArray *modeMessage = [self generateModeMessageCompact:compact layers:layers messageSizeInWords:messageSizeInWords];
  ...
  VZZXBitMatrix *matrix = [[VZZXBitMatrix alloc] initWithDimension:matrixSize];
  ...
  VZZXAztecCode *aztec = [[VZZXAztecCode alloc] init];
...
+ (void)drawBullsEye:(VZZXBitMatrix *)matrix center:(int)center size:(int)size {
...
+ (VZZXBitArray *)generateModeMessageCompact:(BOOL)compact layers:(int)layers messageSizeInWords:(int)messageSizeInWords {
  VZZXBitArray *modeMessage = [[VZZXBitArray alloc] init];
  if (compact) {
...
+ (void)drawModeMessage:(VZZXBitMatrix *)matrix compact:(BOOL)compact matrixSize:(int)matrixSize modeMessage:(VZZXBitArray *)modeMessage {
  int center = matrixSize / 2;
  if (compact) {
    for (int i = 0; i < 7; i++) {
...
+ (VZZXBitArray *)generateCheckWords:(VZZXBitArray *)bitArray totalBits:(int)totalBits wordSize:(int)wordSize {
  // bitArray is guaranteed to be a multiple of the wordSize, so no padding needed
  int messageSizeInWords = bitArray.size / wordSize;
  VZZXReedSolomonEncoder *rs = [[VZZXReedSolomonEncoder alloc] initWithField:[self getGF:wordSize]];
  int totalWords = totalBits / wordSize;

  VZZXIntArray *messageWords = [self bitsToWords:bitArray wordSize:wordSize totalWords:totalWords];
  [rs encode:messageWords ecBytes:totalWords - messageSizeInWords];
  int startPad = totalBits % wordSize;
  VZZXBitArray *messageBits = [[VZZXBitArray alloc] init];
...
+ (VZZXIntArray *)bitsToWords:(VZZXBitArray *)stuffedBits wordSize:(int)wordSize totalWords:(int)totalWords {
  VZZXIntArray *message = [[VZZXIntArray alloc] initWithLength:totalWords];
  int i;
  int n;
...
+ (VZZXGenericGF *)getGF:(int)wordSize {
  switch (wordSize) {
    case 4:
      return [VZZXGenericGF AztecParam];
    case 6:
      return [VZZXGenericGF AztecData6];
    case 8:
      return [VZZXGenericGF AztecData8];
    case 10:
      return [VZZXGenericGF AztecData10];
    case 12:
      return [VZZXGenericGF AztecData12];
    default:
      return nil;
  }
}

+ (VZZXBitArray *)stuffBits:(VZZXBitArray *)bits wordSize:(int)wordSize {
  VZZXBitArray *arrayOut = [[VZZXBitArray alloc] init];
