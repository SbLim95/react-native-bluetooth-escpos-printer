...
- (VZZXResultPoint *)toResultPoint {
  return [[VZZXResultPoint alloc] initWithX:self.x y:self.y];
}
...
@interface VZZXAztecDetector ()

@property (nonatomic, assign, getter = isCompact) BOOL compact;
@property (nonatomic, strong) VZZXBitMatrix *image;
...
@implementation VZZXAztecDetector
...
- (VZZXAztecDetectorResult *)detectWithMirror:(BOOL)isMirror error:(NSError **)error {
  ...
  VZZXAztecPoint *pCenter = [self matrixCenter];
  if (!pCenter) {
  ...
  if (isMirror) {
    VZZXResultPoint *temp = bullsEyeCorners[0];
  ...
  // 4. Sample the grid
  VZZXBitMatrix *bits = [self sampleGrid:self.image
  ...
  return [[VZZXAztecDetectorResult alloc] initWithBits:bits
                                               points:corners
...
- (BOOL)extractParameters:(NSArray *)bullsEyeCorners {
  VZZXResultPoint *p0 = bullsEyeCorners[0];
  VZZXResultPoint *p1 = bullsEyeCorners[1];
  VZZXResultPoint *p2 = bullsEyeCorners[2];
  VZZXResultPoint *p3 = bullsEyeCorners[3];
...
- (int)correctedParameterData:(long)parameterData compact:(BOOL)compact {
  ...
  VZZXIntArray *parameterWords = [[VZZXIntArray alloc] initWithLength:numCodewords];
  for (int i = numCodewords - 1; i >= 0; --i) {
    parameterWords.array[i] = (int32_t) parameterData & 0xF;
    parameterData >>= 4;
  }

  VZZXReedSolomonDecoder *rsDecoder = [[VZZXReedSolomonDecoder alloc] initWithField:[VZZXGenericGF AztecParam]];
 ...
 */
- (NSMutableArray *)bullsEyeCorners:(VZZXAztecPoint *)pCenter {
  VZZXAztecPoint *pina = pCenter;
  VZZXAztecPoint *pinb = pCenter;
  VZZXAztecPoint *pinc = pCenter;
  VZZXAztecPoint *pind = pCenter;

  BOOL color = YES;

  for (self.nbCenterLayers = 1; self.nbCenterLayers < 9; self.nbCenterLayers++) {
    VZZXAztecPoint *pouta = [self firstDifferent:pina color:color dx:1 dy:-1];
    VZZXAztecPoint *poutb = [self firstDifferent:pinb color:color dx:1 dy:1];
    VZZXAztecPoint *poutc = [self firstDifferent:pinc color:color dx:-1 dy:1];
    VZZXAztecPoint *poutd = [self firstDifferent:pind color:color dx:-1 dy:-1];
  ...
  VZZXResultPoint *pinax = [[VZZXResultPoint alloc] initWithX:pina.x + 0.5f y:pina.y - 0.5f];
  VZZXResultPoint *pinbx = [[VZZXResultPoint alloc] initWithX:pinb.x + 0.5f y:pinb.y + 0.5f];
  VZZXResultPoint *pincx = [[VZZXResultPoint alloc] initWithX:pinc.x - 0.5f y:pinc.y + 0.5f];
  VZZXResultPoint *pindx = [[VZZXResultPoint alloc] initWithX:pind.x - 0.5f y:pind.y - 0.5f];
 ...
 */
- (VZZXAztecPoint *)matrixCenter {
  VZZXResultPoint *pointA;
  VZZXResultPoint *pointB;
  VZZXResultPoint *pointC;
  VZZXResultPoint *pointD;

  VZZXWhiteRectangleDetector *detector = [[VZZXWhiteRectangleDetector alloc] initWithImage:self.image error:nil];
  ...
  if (cornerPoints) {
    ...
    pointA = [[self firstDifferent:[[VZZXAztecPoint alloc] initWithX:cx + 7 y:cy - 7] color:NO dx:1 dy:-1] toResultPoint];
    pointB = [[self firstDifferent:[[VZZXAztecPoint alloc] initWithX:cx + 7 y:cy + 7]  color:NO dx:1 dy:1] toResultPoint];
    pointC = [[self firstDifferent:[[VZZXAztecPoint alloc] initWithX:cx - 7 y:cy + 7]  color:NO dx:-1 dy:1] toResultPoint];
    pointD = [[self firstDifferent:[[VZZXAztecPoint alloc] initWithX:cx - 7 y:cy - 7]  color:NO dx:-1 dy:-1] toResultPoint];
  }

  //Compute the center of the rectangle
  int cx = [VZZXMathUtils round:([pointA x] + [pointD x] + [pointB x] + [pointC x]) / 4.0f];
  int cy = [VZZXMathUtils round:([pointA y] + [pointD y] + [pointB y] + [pointC y]) / 4.0f];

  // Redetermine the white rectangle starting from previously computed center.
  // This will ensure that we end up with a white rectangle in center bull's eye
  // in order to compute a more accurate center.
  detector = [[VZZXWhiteRectangleDetector alloc] initWithImage:self.image initSize:15 x:cx y:cy error:nil];
  ...
  cx = [VZZXMathUtils round:([pointA x] + [pointD x] + [pointB x] + [pointC x]) / 4];
  cy = [VZZXMathUtils round:([pointA y] + [pointD y] + [pointB y] + [pointC y]) / 4];

  // Recompute the center of the rectangle
  return [[VZZXAztecPoint alloc] initWithX:cx y:cy];
}
 ...
 */
- (VZZXBitMatrix *)sampleGrid:(VZZXBitMatrix *)anImage
 ...
 */
- (int)sampleLine:(VZZXResultPoint *)p1 p2:(VZZXResultPoint *)p2 size:(int)size {
  int result = 0;
  ...
  for (int i = 0; i < size; i++) {
    if ([self.image getX:[VZZXMathUtils round:px + i * dx] y:[VZZXMathUtils round:py + i * dy]]) {
      result |= 1 << (size - i - 1);
    }
  }
 ...
 */
- (BOOL)isWhiteOrBlackRectangle:(VZZXAztecPoint *)p1 p2:(VZZXAztecPoint *)p2 p3:(VZZXAztecPoint *)p3 p4:(VZZXAztecPoint *)p4 {
  int corr = 3;

  p1 = [[VZZXAztecPoint alloc] initWithX:p1.x - corr y:p1.y + corr];
  p2 = [[VZZXAztecPoint alloc] initWithX:p2.x - corr y:p2.y - corr];
  p3 = [[VZZXAztecPoint alloc] initWithX:p3.x + corr y:p3.y - corr];
  p4 = [[VZZXAztecPoint alloc] initWithX:p4.x + corr y:p4.y + corr];
 ...
 */
- (int)color:(VZZXAztecPoint *)p1 p2:(VZZXAztecPoint *)p2 {
  ...
  for (int i = 0; i < d; i++) {
    px += dx;
    py += dy;
    if ([self.image getX:[VZZXMathUtils round:px] y:[VZZXMathUtils round:py]] != colorModel) {
      error++;
    }
  }
 ...
 */
- (VZZXAztecPoint *)firstDifferent:(VZZXAztecPoint *)init color:(BOOL)color dx:(int)dx dy:(int)dy {
  int x = init.x + dx;
  int y = init.y + dy;
  ...
  return [[VZZXAztecPoint alloc] initWithX:x y:y];
}
...
- (NSArray *)expandSquare:(NSArray *)cornerPoints oldSide:(float)oldSide newSide:(float)newSide {
  VZZXResultPoint *cornerPoints0 = (VZZXResultPoint *)cornerPoints[0];
  VZZXResultPoint *cornerPoints1 = (VZZXResultPoint *)cornerPoints[1];
  VZZXResultPoint *cornerPoints2 = (VZZXResultPoint *)cornerPoints[2];
  VZZXResultPoint *cornerPoints3 = (VZZXResultPoint *)cornerPoints[3];
  ...
  VZZXResultPoint *result0 = [[VZZXResultPoint alloc] initWithX:centerx + ratio * dx y:centery + ratio * dy];
  VZZXResultPoint *result2 = [[VZZXResultPoint alloc] initWithX:centerx - ratio * dx y:centery - ratio * dy];
  ...
  VZZXResultPoint *result1 = [[VZZXResultPoint alloc] initWithX:centerx + ratio * dx y:centery + ratio * dy];
  VZZXResultPoint *result3 = [[VZZXResultPoint alloc] initWithX:centerx - ratio * dx y:centery - ratio * dy];
...
- (BOOL)isValid:(VZZXResultPoint *)point {
  int x = [VZZXMathUtils round:point.x];
  int y = [VZZXMathUtils round:point.y];
  return [self isValidX:x y:y];
}

- (float)distance:(VZZXAztecPoint *)a b:(VZZXAztecPoint *)b {
  return [VZZXMathUtils distance:a.x aY:a.y bX:b.x bY:b.y];
}

- (float)resultDistance:(VZZXResultPoint *)a b:(VZZXResultPoint *)b {
  return [VZZXMathUtils distance:a.x aY:a.y bX:b.x bY:b.y];
}
