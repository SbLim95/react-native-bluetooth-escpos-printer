...
@interface VZZXMultiFormatUPCEANReader ()

@property (nonatomic, strong, readonly) NSMutableArray *readers;

@end

@implementation VZZXMultiFormatUPCEANReader
  ...
  if (self = [super init]) {
    ...
    if (hints != nil) {
      if ([hints containsFormat:kBarcodeFormatEan13]) {
        [_readers addObject:[[VZZXEAN13Reader alloc] init]];
      } else if ([hints containsFormat:kBarcodeFormatUPCA]) {
        [_readers addObject:[[VZZXUPCAReader alloc] init]];
      }

      if ([hints containsFormat:kBarcodeFormatEan8]) {
        [_readers addObject:[[VZZXEAN8Reader alloc] init]];
      }

      if ([hints containsFormat:kBarcodeFormatUPCE]) {
        [_readers addObject:[[VZZXUPCEReader alloc] init]];
      }
    }
...
- (VZZXResult *)decodeRow:(int)rowNumber row:(VZZXBitArray *)row hints:(VZZXDecodeHints *)hints error:(NSError **)error {
  NSRange startGuardPattern = [VZZXUPCEANReader findStartGuardPattern:row error:error];
  if (startGuardPattern.location == NSNotFound) {
    return nil;
  }
  for (VZZXUPCEANReader *reader in self.readers) {
    VZZXResult *result = [reader decodeRow:rowNumber row:row startGuardRange:startGuardPattern hints:hints error:error];
    if (!result) {
      continue;
    }
    ...
    if (ean13MayBeUPCA && canReturnUPCA) {
      ...
      VZZXResult *resultUPCA = [VZZXResult resultWithText:[result.text substringFromIndex:1]
