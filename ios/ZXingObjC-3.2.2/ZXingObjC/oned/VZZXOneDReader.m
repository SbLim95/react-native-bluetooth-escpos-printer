...
- (VZZXResult *)decode:(VZZXBinaryBitmap *)image hints:(VZZXDecodeHints *)hints error:(NSError **)error {
  ...
  VZZXResult *result = [self doDecode:image hints:hints error:&decodeError];
  if (result) {
    return result;
  } else if (decodeError.code == VZZXNotFoundError) {
    BOOL tryHarder = hints != nil && hints.tryHarder;
    if (tryHarder && [image rotateSupported]) {
      VZZXBinaryBitmap *rotatedImage = [image rotateCounterClockwise];
      VZZXResult *result = [self doDecode:rotatedImage hints:hints error:error];
      if (!result) {
        return nil;
      }
      ...
      if (points != nil) {
        ...
        for (int i = 0; i < [points count]; i++) {
          points[i] = [[VZZXResultPoint alloc] initWithX:height - [(VZZXResultPoint *)points[i] y]
                                                     y:[(VZZXResultPoint *)points[i] x]];
        }
      }
      return result;
    }
 ...
 */
- (VZZXResult *)doDecode:(VZZXBinaryBitmap *)image hints:(VZZXDecodeHints *)hints error:(NSError **)error {
  int width = image.width;
  int height = image.height;
  VZZXBitArray *row = [[VZZXBitArray alloc] initWithSize:width];
  int middle = height >> 1;
  BOOL tryHarder = hints != nil && hints.tryHarder;
  ...
  for (int x = 0; x < maxLines; x++) {
    ...
    if (!row && rowError.code == VZZXNotFoundError) {
      continue;
    } else if (!row) {
    ...
    for (int attempt = 0; attempt < 2; attempt++) {
      ...
      VZZXResult *result = [self decodeRow:rowNumber row:row hints:hints error:nil];
      if (result) {
        if (attempt == 1) {
          [result putMetadata:kResultMetadataTypeOrientation value:@180];
          NSMutableArray *points = [result resultPoints];
          if (points != nil) {
            points[0] = [[VZZXResultPoint alloc] initWithX:width - [(VZZXResultPoint *)points[0] x]
                                                       y:[(VZZXResultPoint *)points[0] y]];
            points[1] = [[VZZXResultPoint alloc] initWithX:width - [(VZZXResultPoint *)points[1] x]
                                                       y:[(VZZXResultPoint *)points[1] y]];
          }
        }
        return result;
      }
 ...
 */
+ (BOOL)recordPattern:(VZZXBitArray *)row start:(int)start counters:(VZZXIntArray *)counters {
...
+ (BOOL)recordPatternInReverse:(VZZXBitArray *)row start:(int)start counters:(VZZXIntArray *)counters {
 ...
 */
+ (float)patternMatchVariance:(VZZXIntArray *)counters pattern:(const int[])pattern maxIndividualVariance:(float)maxIndividualVariance {
  int numCounters = counters.length;
  int total = 0;
  int patternLength = 0;
 ...
 * @return VZZXResult containing encoded string and start/end of barcode or nil
