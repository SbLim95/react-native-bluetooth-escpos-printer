...
const unichar VZZX_CODE128_ESCAPE_FNC_1 = L'\u00f1';
const unichar VZZX_CODE128_ESCAPE_FNC_2 = L'\u00f2';
const unichar VZZX_CODE128_ESCAPE_FNC_3 = L'\u00f3';
const unichar VZZX_CODE128_ESCAPE_FNC_4 = L'\u00f4';

@implementation VZZXCode128Writer
...
- (VZZXBoolArray *)encode:(NSString *)contents {
  ...
  for (int i = 0; i < length; i++) {
    ...
    if (c < ' ' || c > '~') {
      switch (c) {
        case VZZX_CODE128_ESCAPE_FNC_1:
        case VZZX_CODE128_ESCAPE_FNC_2:
        case VZZX_CODE128_ESCAPE_FNC_3:
        case VZZX_CODE128_ESCAPE_FNC_4:
          break;
        default:
  ...
  while (position < length) {
    //Select code to use
    int requiredDigitCount = codeSet == VZZX_CODE128_CODE_CODE_C ? 2 : 4;
    int newCodeSet;
    if ([self isDigits:contents start:position length:requiredDigitCount]) {
      newCodeSet = VZZX_CODE128_CODE_CODE_C;
    } else {
      newCodeSet = VZZX_CODE128_CODE_CODE_B;
    }
    ...
    if (newCodeSet == codeSet) {
      ...
      switch ([contents characterAtIndex:position]) {
        case VZZX_CODE128_ESCAPE_FNC_1:
          patternIndex = VZZX_CODE128_CODE_FNC_1;
          break;
        case VZZX_CODE128_ESCAPE_FNC_2:
          patternIndex = VZZX_CODE128_CODE_FNC_2;
          break;
        case VZZX_CODE128_ESCAPE_FNC_3:
          patternIndex = VZZX_CODE128_CODE_FNC_3;
          break;
        case VZZX_CODE128_ESCAPE_FNC_4:
          patternIndex = VZZX_CODE128_CODE_FNC_4_B; // FIXME if this ever outputs Code A
          break;
        default:
          // Then handle normal characters otherwise
          if (codeSet == VZZX_CODE128_CODE_CODE_B) {
      ...
      if (codeSet == 0) {
        ...
        if (newCodeSet == VZZX_CODE128_CODE_CODE_B) {
          patternIndex = VZZX_CODE128_CODE_START_B;
        } else {
          // CODE_CODE_C
          patternIndex = VZZX_CODE128_CODE_START_C;
        }
      } else {
    ...
    for (int i = 0; i < sizeof(VZZX_CODE128_CODE_PATTERNS[patternIndex]) / sizeof(int); i++) {
      [pattern addObject:@(VZZX_CODE128_CODE_PATTERNS[patternIndex][i])];
    }
  ...
  checkSum %= 103;
  NSMutableArray *pattern = [NSMutableArray array];
  for (int i = 0; i < sizeof(VZZX_CODE128_CODE_PATTERNS[checkSum]) / sizeof(int); i++) {
    [pattern addObject:@(VZZX_CODE128_CODE_PATTERNS[checkSum][i])];
  }
  [patterns addObject:pattern];

  // Append stop code
  pattern = [NSMutableArray array];
  for (int i = 0; i < sizeof(VZZX_CODE128_CODE_PATTERNS[VZZX_CODE128_CODE_STOP]) / sizeof(int); i++) {
    [pattern addObject:@(VZZX_CODE128_CODE_PATTERNS[VZZX_CODE128_CODE_STOP][i])];
  }
  [patterns addObject:pattern];
...
- (BOOL)isDigits:(NSString *)value start:(int)start length:(unsigned int)length {
  ...
  for (int i = start; i < end && i < last; i++) {
    ...
    if (c < '0' || c > '9') {
      if (c != VZZX_CODE128_ESCAPE_FNC_1) {
        return NO;
      }
      end++; // ignore FNC_1
    }
