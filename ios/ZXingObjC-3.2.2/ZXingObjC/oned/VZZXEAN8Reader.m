...
- (int)decodeMiddle:(VZZXBitArray *)row startRange:(NSRange)startRange result:(NSMutableString *)result error:(NSError **)error {
  ...
  for (int x = 0; x < 4 && rowOffset < end; x++) {
    int bestMatch = [VZZXUPCEANReader decodeDigit:row counters:counters rowOffset:rowOffset patternType:VZZX_UPC_EAN_PATTERNS_L_PATTERNS error:error];
    if (bestMatch == -1) {
      return -1;
    }
  ...
  NSRange middleRange = [[self class] findGuardPattern:row rowOffset:rowOffset whiteFirst:YES pattern:VZZX_UPC_EAN_MIDDLE_PATTERN patternLen:VZZX_UPC_EAN_MIDDLE_PATTERN_LEN error:error];
