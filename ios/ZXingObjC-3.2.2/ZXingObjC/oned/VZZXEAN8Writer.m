...
const int VZZX_EAN8_CODE_WIDTH = 3 + (7 * 4) + 5 + (7 * 4) + 3;

@implementation VZZXEAN8Writer
...
- (VZZXBoolArray *)encode:(NSString *)contents {
  ...
  VZZXBoolArray *result = [[VZZXBoolArray alloc] initWithLength:VZZX_EAN8_CODE_WIDTH];
  int pos = 0;
  ...
  for (int i = 0; i <= 3; i++) {
    ...
    pos += [self appendPattern:result pos:pos pattern:VZZX_UPC_EAN_L_PATTERNS[digit] patternLen:VZZX_UPC_EAN_L_PATTERNS_SUB_LEN startColor:FALSE];
  }
  ...
  for (int i = 4; i <= 7; i++) {
    int digit = [[contents substringWithRange:NSMakeRange(i, 1)] intValue];
    pos += [super appendPattern:result pos:pos pattern:VZZX_UPC_EAN_L_PATTERNS[digit] patternLen:VZZX_UPC_EAN_L_PATTERNS_SUB_LEN startColor:YES];
  }
