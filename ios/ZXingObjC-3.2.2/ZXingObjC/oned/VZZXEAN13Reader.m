...
//                0    1    1   0   0    1   == 0x19
//
const int VZZX_EAN13_FIRST_DIGIT_ENCODINGS[] = {
  0x00, 0x0B, 0x0D, 0xE, 0x13, 0x19, 0x1C, 0x15, 0x16, 0x1A
};

@interface VZZXEAN13Reader ()

@property (nonatomic, strong, readonly) VZZXIntArray *decodeMiddleCounters;

@end

@implementation VZZXEAN13Reader
...
- (int)decodeMiddle:(VZZXBitArray *)row startRange:(NSRange)startRange result:(NSMutableString *)result error:(NSError **)error {
  ...
  NSRange middleRange = [[self class] findGuardPattern:row
                                             rowOffset:rowOffset
                                            whiteFirst:YES
                                               pattern:VZZX_UPC_EAN_MIDDLE_PATTERN
                                            patternLen:VZZX_UPC_EAN_MIDDLE_PATTERN_LEN
                                                 error:error];
  ...
  for (int x = 0; x < 6 && rowOffset < end; x++) {
    int bestMatch = [VZZXUPCEANReader decodeDigit:row
                                       counters:counters
                                      rowOffset:rowOffset
                                    patternType:VZZX_UPC_EAN_PATTERNS_L_PATTERNS
                                          error:error];
    if (bestMatch == -1) {
      return -1;
    }
...
- (BOOL)determineFirstDigit:(NSMutableString *)resultString lgPatternFound:(int)lgPatternFound {
  for (int d = 0; d < 10; d++) {
    if (lgPatternFound == VZZX_EAN13_FIRST_DIGIT_ENCODINGS[d]) {
