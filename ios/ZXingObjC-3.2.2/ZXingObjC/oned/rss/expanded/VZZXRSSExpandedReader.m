...
#import "VZZXBitArrayBuilder.h"
#import "VZZXErrors.h"
#import "VZZXIntArray.h"
#import "VZZXResult.h"
#import "VZZXRSSDataCharacter.h"
#import "VZZXRSSExpandedPair.h"
#import "VZZXRSSExpandedReader.h"
#import "VZZXRSSExpandedRow.h"
#import "VZZXRSSFinderPattern.h"
#import "VZZXRSSUtils.h"

const int VZZX_SYMBOL_WIDEST[] = {7, 5, 4, 3, 1};
const int VZZX_EVEN_TOTAL_SUBSET[] = {4, 20, 52, 104, 204};
const int VZZX_GSUM[] = {0, 348, 1388, 2948, 3988};

const int VZZX_WEIGHTS[][8] = {
...
const int VZZX_FINDER_PAT_A = 0;
const int VZZX_FINDER_PAT_B = 1;
const int VZZX_FINDER_PAT_C = 2;
const int VZZX_FINDER_PAT_D = 3;
const int VZZX_FINDER_PAT_E = 4;
const int VZZX_FINDER_PAT_F = 5;

#define VZZX_FINDER_PATTERN_SEQUENCES_LEN 10
#define VZZX_FINDER_PATTERN_SEQUENCES_SUBLEN 11
const int VZZX_FINDER_PATTERN_SEQUENCES[VZZX_FINDER_PATTERN_SEQUENCES_LEN][VZZX_FINDER_PATTERN_SEQUENCES_SUBLEN] = {
  { VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_A },
  { VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_B },
  { VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_C, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_D },
  { VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_E, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_D, VZZX_FINDER_PAT_C },
  { VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_E, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_D, VZZX_FINDER_PAT_D, VZZX_FINDER_PAT_F },
  { VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_E, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_D, VZZX_FINDER_PAT_E, VZZX_FINDER_PAT_F, VZZX_FINDER_PAT_F },
  { VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_C, VZZX_FINDER_PAT_C, VZZX_FINDER_PAT_D, VZZX_FINDER_PAT_D },
  { VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_C, VZZX_FINDER_PAT_C, VZZX_FINDER_PAT_D, VZZX_FINDER_PAT_E, VZZX_FINDER_PAT_E },
  { VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_C, VZZX_FINDER_PAT_C, VZZX_FINDER_PAT_D, VZZX_FINDER_PAT_E, VZZX_FINDER_PAT_F, VZZX_FINDER_PAT_F },
  { VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_A, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_B, VZZX_FINDER_PAT_C, VZZX_FINDER_PAT_D, VZZX_FINDER_PAT_D, VZZX_FINDER_PAT_E, VZZX_FINDER_PAT_E, VZZX_FINDER_PAT_F, VZZX_FINDER_PAT_F },
};

@interface VZZXRSSExpandedReader ()

@property (nonatomic, strong, readonly) VZZXIntArray *startEnd;
...
@implementation VZZXRSSExpandedReader
...
- (id)init {
  if (self = [super init]) {
    ...
    _startFromEven = NO;
    _startEnd = [[VZZXIntArray alloc] initWithLength:2];
  }
...
- (VZZXResult *)decodeRow:(int)rowNumber row:(VZZXBitArray *)row hints:(VZZXDecodeHints *)hints error:(NSError **)error {
  ...
  if (pairs) {
    VZZXResult *result = [self constructResult:pairs error:error];
    if (result) {
      return result;
    }
  }
...
- (NSMutableArray *)decodeRow2pairs:(int)rowNumber row:(VZZXBitArray *)row error:(NSError **)error {
  while (YES) {
    VZZXRSSExpandedPair *nextPair = [self retrieveNextPair:row previousPairs:self.pairs rowNumber:rowNumber];
    if (!nextPair) {
...
- (NSMutableArray *)checkRows:(NSMutableArray *)collectedRows current:(int)currentRow {
  for (int i = currentRow; i < [self.rows count]; i++) {
    VZZXRSSExpandedRow *row = self.rows[i];
    [self.pairs removeAllObjects];
...
- (BOOL)isValidSequence:(NSArray *)pairs {
  ...
  for (int i = 0, sz = 2; i < VZZX_FINDER_PATTERN_SEQUENCES_LEN; i++, sz++) {
    if (count > sz) {
      continue;
    }
    ...
    for (int j = 0; j < count; j++) {
      if ([[pairs[j] finderPattern] value] != VZZX_FINDER_PATTERN_SEQUENCES[i][j]) {
        stop = NO;
        break;
      }
    }
...
- (void)storeRow:(int)rowNumber wasReversed:(BOOL)wasReversed {
  ...
  while (insertPos < [self.rows count]) {
    VZZXRSSExpandedRow *erow = self.rows[insertPos];
  ...
  [self.rows insertObject:[[VZZXRSSExpandedRow alloc] initWithPairs:self.pairs rowNumber:rowNumber wasReversed:wasReversed] atIndex:insertPos];
...
- (void)removePartialRows:(NSArray *)pairs from:(NSMutableArray *)rows {
  ...
  for (VZZXRSSExpandedRow *r in rows) {
    if ([r.pairs count] == [pairs count]) {
      continue;
    }
    BOOL allFound = YES;
    for (VZZXRSSExpandedPair *p in r.pairs) {
      BOOL found = NO;
      for (VZZXRSSExpandedPair *pp in pairs) {
        if ([p isEqual:pp]) {
          found = YES;
          break;
        }
  ...
  for (VZZXRSSExpandedRow *r in toRemove) {
    [rows removeObject:r];
  }
}
...
- (BOOL)isPartialRow:(NSArray *)pairs of:(NSArray *)rows {
  for (VZZXRSSExpandedRow *r in rows) {
		BOOL allFound = YES;
    for (VZZXRSSExpandedPair *p in pairs) {
      BOOL found = NO;
      for (VZZXRSSExpandedPair *pp in r.pairs) {
        if ([p isEqual:pp]) {
          found = YES;
          break;
        }
...
- (VZZXResult *)constructResult:(NSMutableArray *)pairs error:(NSError **)error {
  VZZXBitArray *binary = [VZZXBitArrayBuilder buildBitArray:pairs];

  VZZXAbstractExpandedDecoder *decoder = [VZZXAbstractExpandedDecoder createDecoder:binary];
  NSString *resultingString = [decoder parseInformationWithError:error];
  if (!resultingString) {
    return nil;
  }

  NSArray *firstPoints = [[((VZZXRSSExpandedPair *)_pairs[0]) finderPattern] resultPoints];
  NSArray *lastPoints = [[((VZZXRSSExpandedPair *)[_pairs lastObject]) finderPattern] resultPoints];

  return [VZZXResult resultWithText:resultingString
                         rawBytes:nil
...
- (BOOL)checkChecksum {
  VZZXRSSExpandedPair *firstPair = self.pairs[0];
  VZZXRSSDataCharacter *checkCharacter = firstPair.leftChar;
  VZZXRSSDataCharacter *firstCharacter = firstPair.rightChar;
  ...
  for (int i = 1; i < self.pairs.count; ++i) {
    VZZXRSSExpandedPair *currentPair = self.pairs[i];
    checksum += currentPair.leftChar.checksumPortion;
    s++;
    VZZXRSSDataCharacter *currentRightChar = currentPair.rightChar;
...
- (int)nextSecondBar:(VZZXBitArray *)row initialPos:(int)initialPos {
  int currentPos;
...
- (VZZXRSSExpandedPair *)retrieveNextPair:(VZZXBitArray *)row previousPairs:(NSMutableArray *)previousPairs rowNumber:(int)rowNumber {
  BOOL isOddPattern = [previousPairs count] % 2 == 0;
  if (self.startFromEven) {
    isOddPattern = !isOddPattern;
  }

  VZZXRSSFinderPattern *pattern;
  ...
  VZZXRSSDataCharacter *leftChar = [self decodeDataCharacter:row pattern:pattern isOddPattern:isOddPattern leftChar:YES];
  if (!leftChar) {
    return nil;
  }
  ...
  VZZXRSSDataCharacter *rightChar = [self decodeDataCharacter:row pattern:pattern isOddPattern:isOddPattern leftChar:NO];
  BOOL mayBeLast = YES;
  return [[VZZXRSSExpandedPair alloc] initWithLeftChar:leftChar rightChar:rightChar finderPattern:pattern mayBeLast:mayBeLast];
}

- (BOOL)findNextPair:(VZZXBitArray *)row previousPairs:(NSMutableArray *)previousPairs forcedOffset:(int)forcedOffset {
  ...
  if (forcedOffset >= 0) {
    ...
    VZZXRSSExpandedPair *lastPair = [previousPairs lastObject];
...
- (void)reverseCounters:(VZZXIntArray *)counters {
...
- (VZZXRSSFinderPattern *)parseFoundFinderPattern:(VZZXBitArray *)row rowNumber:(int)rowNumber oddPattern:(BOOL)oddPattern {
  ...
  VZZXIntArray *counters = [[VZZXIntArray alloc] initWithLength:self.decodeFinderCounters.length];
  ...
  int value = [VZZXAbstractRSSReader parseFinderValue:counters finderPatternType:VZZX_RSS_PATTERNS_RSS_EXPANDED_PATTERNS];
  if (value == -1) {
    return nil;
  }
  return [[VZZXRSSFinderPattern alloc] initWithValue:value startEnd:[[VZZXIntArray alloc] initWithInts:start, end, -1] start:start end:end rowNumber:rowNumber];
}

- (VZZXRSSDataCharacter *)decodeDataCharacter:(VZZXBitArray *)row pattern:(VZZXRSSFinderPattern *)pattern isOddPattern:(BOOL)isOddPattern leftChar:(BOOL)leftChar {
  ...
  for (int i = self.oddCounts.length - 1; i >= 0; i--) {
    if ([self isNotA1left:pattern isOddPattern:isOddPattern leftChar:leftChar]) {
      int weight = VZZX_WEIGHTS[weightRowNumber][2 * i];
  ...
  for (int i = self.evenCounts.length - 1; i >= 0; i--) {
    if ([self isNotA1left:pattern isOddPattern:isOddPattern leftChar:leftChar]) {
      int weight = VZZX_WEIGHTS[weightRowNumber][2 * i + 1];
  ...
  int oddWidest = VZZX_SYMBOL_WIDEST[group];
  ...
  int tEven = VZZX_EVEN_TOTAL_SUBSET[group];
  int gSum = VZZX_GSUM[group];
  int value = vOdd * tEven + vEven + gSum;
  return [[VZZXRSSDataCharacter alloc] initWithValue:value checksumPortion:checksumPortion];
}

- (BOOL)isNotA1left:(VZZXRSSFinderPattern *)pattern isOddPattern:(BOOL)isOddPattern leftChar:(BOOL)leftChar {
