...
#import "VZZXErrors.h"
#import "VZZXRSSExpandedBlockParsedResult.h"
#import "VZZXRSSExpandedCurrentParsingState.h"
#import "VZZXRSSExpandedDecodedChar.h"
#import "VZZXRSSExpandedDecodedInformation.h"
#import "VZZXRSSExpandedDecodedNumeric.h"
#import "VZZXRSSExpandedFieldParser.h"
#import "VZZXRSSExpandedGeneralAppIdDecoder.h"

@interface VZZXRSSExpandedGeneralAppIdDecoder ()

@property (nonatomic, strong, readonly) VZZXBitArray *information;
@property (nonatomic, strong, readonly) VZZXRSSExpandedCurrentParsingState *current;
@property (nonatomic, strong, readonly) NSMutableString *buffer;

@end

@implementation VZZXRSSExpandedGeneralAppIdDecoder

- (id)initWithInformation:(VZZXBitArray *)information {
  if (self = [super init]) {
    _current = [[VZZXRSSExpandedCurrentParsingState alloc] init];
...
- (NSString *)decodeAllCodes:(NSMutableString *)buff initialPosition:(int)initialPosition error:(NSError **)error {
  ...
  do {
    VZZXRSSExpandedDecodedInformation *info = [self decodeGeneralPurposeField:currentPosition remaining:remaining];
    if (!info) {
      if (error) *error = VZZXFormatErrorInstance();
      return nil;
    }
    NSString *parsedFields = [VZZXRSSExpandedFieldParser parseFieldsInGeneralPurpose:[info theNewString] error:error];
    if (!parsedFields) {
      return nil;
...
- (VZZXRSSExpandedDecodedNumeric *)decodeNumeric:(int)pos {
  if (pos + 7 > self.information.size) {
    int numeric = [self extractNumericValueFromBitArray:pos bits:4];
    if (numeric == 0) {
      return [[VZZXRSSExpandedDecodedNumeric alloc] initWithNewPosition:self.information.size
                                                 firstDigit:VZZX_FNC1_INT
                                                secondDigit:VZZX_FNC1_INT];
    }
    return [[VZZXRSSExpandedDecodedNumeric alloc] initWithNewPosition:self.information.size
  ...
  return [[VZZXRSSExpandedDecodedNumeric alloc] initWithNewPosition:pos + 7
                                             firstDigit:digit1
                                            secondDigit:digit2];
}

- (int)extractNumericValueFromBitArray:(int)pos bits:(int)bits {
  return [VZZXRSSExpandedGeneralAppIdDecoder extractNumericValueFromBitArray:self.information pos:pos bits:bits];
}

+ (int)extractNumericValueFromBitArray:(VZZXBitArray *)information pos:(int)pos bits:(int)bits {
  if (bits > 32) {
...
- (VZZXRSSExpandedDecodedInformation *)decodeGeneralPurposeField:(int)pos remaining:(NSString *)remaining {
  ...
  NSError *error;
  VZZXRSSExpandedDecodedInformation *lastDecoded = [self parseBlocksWithError:&error];
  if (error) {
    return nil;
  }

  if (lastDecoded != nil && [lastDecoded remaining]) {
    return [[VZZXRSSExpandedDecodedInformation alloc] initWithNewPosition:self.current.position
                                                    newString:self.buffer
                                               remainingValue:lastDecoded.remainingValue];
  }
  return [[VZZXRSSExpandedDecodedInformation alloc] initWithNewPosition:self.current.position newString:self.buffer];
}

- (VZZXRSSExpandedDecodedInformation *)parseBlocksWithError:(NSError **)error {
  BOOL isFinished;
  VZZXRSSExpandedBlockParsedResult *result;
  do {
...
- (VZZXRSSExpandedBlockParsedResult *)parseNumericBlockWithError:(NSError **)error {
  while ([self isStillNumeric:self.current.position]) {
    VZZXRSSExpandedDecodedNumeric *numeric = [self decodeNumeric:self.current.position];
    if (!numeric) {
      if (error) *error = VZZXFormatErrorInstance();
      return nil;
    }
    ...
    if ([numeric firstDigitFNC1]) {
      VZZXRSSExpandedDecodedInformation *information;
      if ([numeric secondDigitFNC1]) {
        information = [[VZZXRSSExpandedDecodedInformation alloc] initWithNewPosition:self.current.position
                                                              newString:self.buffer];
      } else {
        information = [[VZZXRSSExpandedDecodedInformation alloc] initWithNewPosition:self.current.position
                                                              newString:self.buffer
      ...
      return [[VZZXRSSExpandedBlockParsedResult alloc] initWithInformation:information finished:YES];
    }
    [self.buffer appendFormat:@"%d", numeric.firstDigit];

    if (numeric.secondDigitFNC1) {
      VZZXRSSExpandedDecodedInformation *information = [[VZZXRSSExpandedDecodedInformation alloc] initWithNewPosition:self.current.position
                                                                                  newString:self.buffer];
      return [[VZZXRSSExpandedBlockParsedResult alloc] initWithInformation:information finished:YES];
    }
  ...
  return [[VZZXRSSExpandedBlockParsedResult alloc] initWithFinished:NO];
}

- (VZZXRSSExpandedBlockParsedResult *)parseIsoIec646BlockWithError:(NSError **)error {
  while ([self isStillIsoIec646:self.current.position]) {
    VZZXRSSExpandedDecodedChar *iso = [self decodeIsoIec646:self.current.position];
    if (!iso) {
      if (error) *error = VZZXFormatErrorInstance();
      return nil;
    }
...
- (VZZXRSSExpandedBlockParsedResult *)parseAlphaBlock {
  while ([self isStillAlpha:self.current.position]) {
    VZZXRSSExpandedDecodedChar *alpha = [self decodeAlphanumeric:self.current.position];
...
- (VZZXRSSExpandedDecodedChar *)decodeIsoIec646:(int)pos {
  int fiveBitValue = [self extractNumericValueFromBitArray:pos bits:5];
  if (fiveBitValue == 15) {
    return [[VZZXRSSExpandedDecodedChar alloc] initWithNewPosition:pos + 5 value:VZZX_FNC1_CHAR];
  }

  if (fiveBitValue >= 5 && fiveBitValue < 15) {
    return [[VZZXRSSExpandedDecodedChar alloc] initWithNewPosition:pos + 5 value:(unichar)('0' + fiveBitValue - 5)];
  }

  int sevenBitValue = [self extractNumericValueFromBitArray:pos bits:7];

  if (sevenBitValue >= 64 && sevenBitValue < 90) {
    return [[VZZXRSSExpandedDecodedChar alloc] initWithNewPosition:pos + 7 value:(unichar)(sevenBitValue + 1)];
  }

  if (sevenBitValue >= 90 && sevenBitValue < 116) {
    return [[VZZXRSSExpandedDecodedChar alloc] initWithNewPosition:pos + 7 value:(unichar)(sevenBitValue + 7)];
  }
  ...
  return [[VZZXRSSExpandedDecodedChar alloc] initWithNewPosition:pos + 8 value:c];
}
...
- (VZZXRSSExpandedDecodedChar *)decodeAlphanumeric:(int)pos {
  int fiveBitValue = [self extractNumericValueFromBitArray:pos bits:5];
  if (fiveBitValue == 15) {
    return [[VZZXRSSExpandedDecodedChar alloc] initWithNewPosition:pos + 5 value:VZZX_FNC1_CHAR];
  }
  ...
  if (sixBitValue >= 32 && sixBitValue < 58) {
    return [[VZZXRSSExpandedDecodedChar alloc] initWithNewPosition:pos + 6 value:(unichar)(sixBitValue + 33)];
  }
  ...
  return [[VZZXRSSExpandedDecodedChar alloc] initWithNewPosition:pos + 6 value:c];
}
