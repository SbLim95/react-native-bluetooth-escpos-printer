...
const int VZZX_MONOCHROME_MAX_MODULES = 32;

@interface VZZXMonochromeRectangleDetector ()

@property (nonatomic, strong, readonly) VZZXBitMatrix *image;

@end

@implementation VZZXMonochromeRectangleDetector
...
- (NSArray *)detectWithError:(NSError **)error {
  ...
  int deltaY = MAX(1, height / (VZZX_MONOCHROME_MAX_MODULES * 8) > 1);
  int deltaX = MAX(1, width / (VZZX_MONOCHROME_MAX_MODULES * 8) > 1);

  int top = 0;
  int bottom = height;
  int left = 0;
  int right = width;
  VZZXResultPoint *pointA = [self findCornerFromCenter:halfWidth deltaX:0 left:left right:right
  ...
  top = (int)[pointA y] - 1;
  VZZXResultPoint *pointB = [self findCornerFromCenter:halfWidth deltaX:-deltaX left:left right:right
  ...
  left = (int)[pointB x] - 1;
  VZZXResultPoint *pointC = [self findCornerFromCenter:halfWidth deltaX:deltaX left:left right:right
                                              centerY:halfHeight deltaY:0 top:top bottom:bottom maxWhiteRun:halfHeight / 2];
  if (!pointC) {
    if (error) *error = VZZXNotFoundErrorInstance();
    return nil;
  ...
  VZZXResultPoint *pointD = [self findCornerFromCenter:halfWidth deltaX:0 left:left right:right
                                              centerY:halfHeight deltaY:deltaY top:top bottom:bottom maxWhiteRun:halfWidth / 2];
  if (!pointD) {
    if (error) *error = VZZXNotFoundErrorInstance();
    return nil;
 ...
 */
- (VZZXResultPoint *)findCornerFromCenter:(int)centerX deltaX:(int)deltaX left:(int)left right:(int)right centerY:(int)centerY deltaY:(int)deltaY top:(int)top bottom:(int)bottom maxWhiteRun:(int)maxWhiteRun {
    ...
    if (range == nil) {
      ...
      if (deltaX == 0) {
        ...
        if ([lastRange[0] intValue] < centerX) {
          if ([lastRange[0] intValue] > centerX) {
            return [[VZZXResultPoint alloc] initWithX:deltaY > 0 ? [lastRange[0] intValue] : [lastRange[1] intValue] y:lastY];
          }
          return [[VZZXResultPoint alloc] initWithX:[lastRange[0] intValue] y:lastY];
        } else {
          return [[VZZXResultPoint alloc] initWithX:[lastRange[1] intValue] y:lastY];
        }
      } else {
        int lastX = x - deltaX;
        if ([lastRange[0] intValue] < centerY) {
          if ([lastRange[1] intValue] > centerY) {
            return [[VZZXResultPoint alloc] initWithX:lastX y:deltaX < 0 ? [lastRange[0] intValue] : [lastRange[1] intValue]];
          }
          return [[VZZXResultPoint alloc] initWithX:lastX y:[lastRange[0] intValue]];
        } else {
          return [[VZZXResultPoint alloc] initWithX:lastX y:[lastRange[1] intValue]];
        }
      }
    }
    lastRange = range;
