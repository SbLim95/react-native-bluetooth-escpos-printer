...
@implementation VZZXCharacterSetECI

+ (void)initialize {
  if ([self class] != [VZZXCharacterSetECI class]) return;
...
+ (void)addCharacterSet:(int)value encoding:(NSStringEncoding)encoding {
  VZZXCharacterSetECI *eci = [[VZZXCharacterSetECI alloc] initWithValue:value encoding:encoding];
  VALUE_TO_ECI[@(value)] = eci;
  ENCODING_TO_ECI[@(encoding)] = eci;
}

+ (VZZXCharacterSetECI *)characterSetECIByValue:(int)value {
  if (VALUE_TO_ECI == nil) {
    [self initialize];
  }
...
+ (VZZXCharacterSetECI *)characterSetECIByEncoding:(NSStringEncoding)encoding {
