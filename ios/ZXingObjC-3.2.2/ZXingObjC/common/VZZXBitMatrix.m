...
@interface VZZXBitMatrix ()

@property (nonatomic, assign, readonly) int bitsSize;

@end

@implementation VZZXBitMatrix
...
+ (VZZXBitMatrix *)parse:(NSString *)stringRepresentation
  ...
  VZZXBoolArray *bits = [[VZZXBoolArray alloc] initWithLength:(unsigned int)stringRepresentation.length];
  int bitsPos = 0;
  int rowStartPos = 0;
  int rowLength = -1;
  int nRows = 0;
  ...
  VZZXBitMatrix *matrix = [[VZZXBitMatrix alloc] initWithWidth:rowLength height:nRows];
...
- (void)xor:(VZZXBitMatrix *)mask {
  ...
  VZZXBitArray *rowArray = [[VZZXBitArray alloc] initWithSize:self.width];
...
- (VZZXBitArray *)rowAtY:(int)y row:(VZZXBitArray *)row {
  if (row == nil || [row size] < self.width) {
    row = [[VZZXBitArray alloc] initWithSize:self.width];
  } else {
    [row clear];
  }
  int offset = y * self.rowSize;
...
- (void)setRowAtY:(int)y row:(VZZXBitArray *)row {
...
- (void)rotate180 {
  ...
  VZZXBitArray *topRow = [[VZZXBitArray alloc] initWithSize:width];
  VZZXBitArray *bottomRow = [[VZZXBitArray alloc] initWithSize:width];
...
- (VZZXIntArray *)enclosingRectangle {
  int left = self.width;
  int top = self.height;
  int right = -1;
  int bottom = -1;
  ...
  return [[VZZXIntArray alloc] initWithInts:left, top, width, height, -1];
}

- (VZZXIntArray *)topLeftOnBit {
  int bitsOffset = 0;
  ...
  x += bit;
  return [[VZZXIntArray alloc] initWithInts:x, y, -1];
}

- (VZZXIntArray *)bottomRightOnBit {
...
- (BOOL)isEqual:(NSObject *)o {
  if (!([o isKindOfClass:[VZZXBitMatrix class]])) {
    return NO;
  }
  VZZXBitMatrix *other = (VZZXBitMatrix *)o;
...
- (id)copyWithZone:(NSZone *)zone {
  return [[VZZXBitMatrix allocWithZone:zone] initWithWidth:self.width height:self.height rowSize:self.rowSize bits:self.bits];
}
