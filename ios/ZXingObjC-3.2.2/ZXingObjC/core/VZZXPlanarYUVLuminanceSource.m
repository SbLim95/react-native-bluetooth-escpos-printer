...
const int VZTHUMBNAIL_SCALE_FACTOR = 2;

@interface VZZXPlanarYUVLuminanceSource ()

@property (nonatomic, strong, readonly) VZZXByteArray *yuvData;
...
@implementation VZZXPlanarYUVLuminanceSource
...
- (id)initWithYuvData:(int8_t *)yuvData yuvDataLen:(int)yuvDataLen dataWidth:(int)dataWidth
  ...
  if (self = [super initWithWidth:width height:height]) {
    ...
    _yuvData = [[VZZXByteArray alloc] initWithLength:yuvDataLen];
...
- (VZZXByteArray *)rowAtY:(int)y row:(VZZXByteArray *)row {
  ...
  if (!row || row.length < width) {
    row = [[VZZXByteArray alloc] initWithLength:width];
  }
...
- (VZZXByteArray *)matrix {
  ...
  VZZXByteArray *matrix = [[VZZXByteArray alloc] initWithLength:area];
  ...
  VZZXByteArray *yuvData = self.yuvData;
...
- (int *)renderThumbnail {
  int thumbWidth = self.width / VZTHUMBNAIL_SCALE_FACTOR;
  int thumbHeight = self.height / VZTHUMBNAIL_SCALE_FACTOR;
  ...
  for (int y = 0; y < self.height; y++) {
    ...
    for (int x = 0; x < self.width; x++) {
      int grey = self.yuvData.array[inputOffset + x * VZTHUMBNAIL_SCALE_FACTOR] & 0xff;
      pixels[outputOffset + x] = 0xFF000000 | (grey * 0x00010101);
    }
    inputOffset += self.dataWidth * VZTHUMBNAIL_SCALE_FACTOR;
  }
  return pixels;
}

- (int)thumbnailWidth {
  return self.width / VZTHUMBNAIL_SCALE_FACTOR;
}

- (int)thumbnailHeight {
  return self.height / VZTHUMBNAIL_SCALE_FACTOR;
}
