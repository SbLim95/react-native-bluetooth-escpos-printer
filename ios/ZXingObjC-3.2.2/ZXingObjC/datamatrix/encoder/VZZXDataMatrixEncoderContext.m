...
@interface VZZXDataMatrixEncoderContext ()

@property (nonatomic, strong) VZZXDimension *maxSize;
@property (nonatomic, strong) VZZXDimension *minSize;

@end

@implementation VZZXDataMatrixEncoderContext
...
- (id)initWithMessage:(NSString *)msg {
  if (self = [super init]) {
    ...
    _symbolShape = VZZXDataMatrixSymbolShapeHintForceNone;
...
- (void)setSizeConstraints:(VZZXDimension *)minSize maxSize:(VZZXDimension *)maxSize {
...
- (void)updateSymbolInfoWithLength:(int)len {
  if (self.symbolInfo == nil || len > self.symbolInfo.dataCapacity) {
    self.symbolInfo = [VZZXDataMatrixSymbolInfo lookup:len shape:self.symbolShape minSize:self.minSize maxSize:self.maxSize fail:YES];
  }
}
