...
@implementation VZZXDataMatrixX12Encoder

- (int)encodingMode {
  return [VZZXDataMatrixHighLevelEncoder x12Encodation];
}
...
- (int)encodeChar:(unichar)c buffer:(NSMutableString *)sb {
  if (c == '\r') {
    ...
    [VZZXDataMatrixHighLevelEncoder illegalCharacter:c];
  }
  return 1;
}
...
- (void)handleEOD:(VZZXDataMatrixEncoderContext *)context buffer:(NSMutableString *)buffer {
  ...
  if (context.remainingCharacters > 1 || available > 1 ||
    ...
    [context writeCodeword:[VZZXDataMatrixHighLevelEncoder x12Unlatch]];
  }
