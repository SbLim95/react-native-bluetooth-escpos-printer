...
@interface VZZXDataMatrixBitMatrixParser ()

@property (nonatomic, strong, readonly) VZZXBitMatrix *mappingBitMatrix;
@property (nonatomic, strong, readonly) VZZXBitMatrix *readMappingMatrix;

@end

@implementation VZZXDataMatrixBitMatrixParser
...
- (id)initWithBitMatrix:(VZZXBitMatrix *)bitMatrix error:(NSError **)error {
  if (self = [super init]) {
    ...
    _readMappingMatrix = [[VZZXBitMatrix alloc] initWithWidth:_mappingBitMatrix.width
 ...
 *
 * @param bitMatrix Original VZZXBitMatrix including alignment patterns
 * @return VZZXDatamatrixVersion encapsulating the Data Matrix Code's "version"
 *  or nil if the dimensions of the mapping matrix are not valid
 *  Data Matrix dimensions.
 */
- (VZZXDataMatrixVersion *)readVersion:(VZZXBitMatrix *)bitMatrix {
  int numRows = bitMatrix.height;
  int numColumns = bitMatrix.width;
  return [VZZXDataMatrixVersion versionForDimensions:numRows numColumns:numColumns];
}

- (VZZXByteArray *)readCodewords {
  VZZXByteArray *result = [[VZZXByteArray alloc] initWithLength:self.version.totalCodewords];
  int resultOffset = 0;
...
/**
 * Extracts the data region from a VZZXBitMatrix that contains
 * alignment patterns.
 *
 * @param bitMatrix Original VZZXBitMatrix with alignment patterns
 * @return BitMatrix that has the alignment patterns removed
 */
- (VZZXBitMatrix *)extractDataRegion:(VZZXBitMatrix *)bitMatrix {
  ...
  VZZXBitMatrix *bitMatrixWithoutAlignment = [[VZZXBitMatrix alloc] initWithWidth:sizeDataRegionColumn height:sizeDataRegionRow];
